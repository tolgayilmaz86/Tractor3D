...
class Terrain : public Ref, public Drawable, public Transform::Listener
{
  ...
  public:
     ...
     * @param scale A scale to apply to the terrain along the X, Y and Z axes. The terrain and any
     * associated physics heightfield is scaled by this amount. Pass Vector3::one() to use the exact
     * dimensions and heights in the supplied height array.
     * @param patchSize Size of terrain patches (number of quads).
     * @param detailLevels Number of detail levels to generate for the terrain (a value of one
     * generates only the base level, resulting in no LOD at runtime.
     * @param skirtScale A positive value indicates that vertical skirts should be generated at the
     * specified scale, which is relative to the height of the terrain. For example, a value of 0.5f
     * indicates that vertical skirts should extend down by half of the maximum height of the
     * terrain. A value of zero disables vertical skirts.
     * @param normalMapPath Path to an object-space normal map to use for terrain lighting, instead
     * of vertex normals.
     * @param materialPath Optional path to a material file to use for the terrain (if not
     * specified, looks for a material file at res/materials/terrain.material.
     *
     * @return A new Terrain.
     * @script{create}
     */
    ...
    bool setLayer(int index,
                  ...
                  const Vector2& textureRepeat = Vector2::one(),
                  const std::string& blendPath = EMPTY_STRING,
                  int blendChannel = 0,
                  int row = -1,
                  int column = -1);
