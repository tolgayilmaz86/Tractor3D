...
namespace tractor
{
	...
	public:
		...
		static Terrain* create(HeightField* heightfield, const Vector3& scale = Vector3::one(), unsigned int patchSize = 32,
			unsigned int detailLevels = 1, float skirtScale = 0.0f, const std::string& normalMapPath = EMPTY_STRING,
		 ...
		 * @script{ignore}
		 */
		bool setLayer(int index, const std::string& texturePath, const Vector2& textureRepeat = Vector2::one(),
			const std::string& blendPath = EMPTY_STRING, int blendChannel = 0,
