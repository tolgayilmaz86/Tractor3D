...
class PhysicsRigidBody : public PhysicsCollisionObject, public Transform::Listener
{
  ...
  public:
    ...
    struct Parameters
    {
         ...
         */
        Parameters()
            : mass(0.0f), friction(0.5f), restitution(0.0f), linearDamping(0.0f),
              angularDamping(0.0f), kinematic(false), anisotropicFriction(Vector3::one()),
              linearFactor(Vector3::one()), angularFactor(Vector3::one())
        {
        }
        ...
        Parameters(float mass,
            ...
            : mass(mass), friction(friction), restitution(restitution),
              linearDamping(linearDamping), angularDamping(angularDamping), kinematic(kinematic),
              anisotropicFriction(anisotropicFriction), linearFactor(linearFactor),
  ...
  private:
     ...
     *
     * @param node The node to create a rigid body for; note that the node must have a model attached
     * to it prior to creating a rigid body for it.
     ...
     */
    static PhysicsRigidBody* create(Node* node,
                                    Properties* properties,
                                    const std::string& nspace = "RIGID_BODY");
