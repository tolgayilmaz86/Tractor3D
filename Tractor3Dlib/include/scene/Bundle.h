...
class Bundle : public Ref
{
  ...
  private:
    ...
    /**
     * Returns the ID of the object at the given file offset by searching through the reference
     ...
     * @return True if successful, false if an error occurred.
     */
    template <class T> bool readArray(unsigned int* length, T** ptr);

    /**
     * Reads an array of values and the array length from the current file position.
     *
     * @param length A pointer to where the length of the array will be copied to.
     * @param values A pointer to the vector to copy the values to. The vector will be resized if it
     * is smaller than length.
     *
     * @return True if successful, false if an error occurred.
     */
    template <class T> bool readArray(unsigned int* length, std::vector<T>* values);
     ...
     * @param values A pointer to the vector to copy the values to. The vector will be resized if it
     * is smaller than length.
     * @param readSize The size that reads will be performed at, size must be the same as or smaller
     * then the sizeof(T)
     *
     * @return True if successful, false if an error occurred.
     */
     ...
     *
     * Note: this is used by Bundle::loadNode(const char*, Scene*) and
     * Bundle::readAnimationChannel(Scene*, Animation*, const char*).
     *
     * @param animation The animation to the load channel into.
