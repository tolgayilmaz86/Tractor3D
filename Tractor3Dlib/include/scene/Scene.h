...
class Scene : public Ref
{
  public:
     ...
     */
    Node* findNode(const std::string& id, bool recursive = true, bool exactMatch = true) const;
     ...
     * @script{ignore}
     */
    unsigned int findNodes(const std::string& id,
     ...
     */
    template <class T> void visit(T* instance, bool (T::*visitMethod)(Node*));
     ...
     */
    template <class T, class C> void visit(T* instance, bool (T::*visitMethod)(Node*, C), C cookie);
  ...
  private:
     ...
     */
    template <class T> void visitNode(Node* node, T* instance, bool (T::*visitMethod)(Node*));
...
template <class T> void Scene::visitNode(Node* node, T* instance, bool (T::*visitMethod)(Node*))
{
    // Invoke the visit method for this node.
    if (!(instance->*visitMethod)(node)) return;
...
{
    // Invoke the visit method for this node.
    if (!(instance->*visitMethod)(node, cookie)) return;
