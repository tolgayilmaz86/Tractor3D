...
namespace tractor
{
	...
	private:
		...
		void addSceneAnimation(const std::string& animationID, const std::string& targetID, const std::string& url);

		void addSceneNodeProperty(SceneNode& sceneNode, SceneNodeProperty::Type type, const std::string& value = EMPTY_STRING, bool supportsUrl = false, int index = 0);
