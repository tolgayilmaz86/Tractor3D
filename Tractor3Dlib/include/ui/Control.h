...
class Control : public Ref, public AnimationTarget, public ScriptTarget
{
  ...
  public:
    ...
    class Listener
    {
      public:
        ...
        enum EventType
        {
            ...
            /**
             * Event triggered when a control is activated in another manner (such as pressing enter
             * in text control)
             */
            ACTIVATED = 0x80,
     ...
     *
     * If the value is passed as a percentage of its parent container's clip region, it is
     * interpreted as a value between 0-1, where 1 equals the full size of it's parent.
     *
     ...
     *               One or more members of the Control::State enum, ORed together.
     */
    void setImageRegion(const std::string& id,
     ...
     */
    const Rectangle& getImageRegion(const std::string& id, State state) const;
     ...
     *               One or more members of the Control::State enum, ORed together.
     */
    void setImageColor(const std::string& id, const Vector4& color, unsigned char states = STATE_ALL);
     ...
     */
    const Vector4& getImageColor(const std::string& id, State state) const;
     ...
     */
    const Theme::UVs& getImageUVs(const std::string& id, State state) const;
     ...
     * @param states The states to set this property on. One or more members of the Control::State
     * enum, OR'ed together.
     */
  ...
  protected:
     ...
     */
    Theme::ThemeImage* getImage(const std::string& id, State state);
     ...
     */
    static bool parseCoordPair(const std::string&,
                               float* v1,
                               float* v2,
                               bool* v1Percentage,
                               bool* v2Percentage);
