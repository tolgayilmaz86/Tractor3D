 ...
 */
template <typename T> static inline std::string toString(T value)
{
    std::stringstream ss;
    ss << value;
 ...
 */
template <typename T> static inline T parseString(const std::string& str)
{
    T result;
...
/**
 * @brief Converts the first character of a string to uppercase letter and lowercases all other
 * characters, if any.
 * @param str - input string to be capitalized.
 * @return A string with the first letter capitalized and all other characters lowercased. It
...
/**
 * @brief Converts only the first character of a string to uppercase letter, all other characters
 * stay unchanged.
 * @param str - input string to be modified.
 * @return A string with the first letter capitalized. All other characters stay unchanged. It
 * doesn't modify the input string.
 */
static inline std::string capitalizeFirstChar(const std::string& str)
{
 ...
 * @return Copy of input str with trimmed white spaces.
 */
static inline std::string trimCcopy(std::string str)
{
    trim(str);
    return str;
}
 ...
 */
static inline bool replaceFirst(std::string& str,
 ...
 * @return True if replacement was successfull, false otherwise.
 */
static inline bool replaceLast(std::string& str,
                               const std::string& target,
                               const std::string& replacement)
{
 ...
 * @return True if replacement was successfull, false otherwise.
 */
static inline bool replaceAll(std::string& str,
                              const std::string& target,
                              const std::string& replacement)
{
    if (target.empty())
 ...
 */
static inline std::vector<std::string> regexSplit(const std::string& src, const std::string& rgx_str)
{
 ...
 */
static inline std::map<std::string, std::string> regexSplitMap(const std::string& src,
 ...
 * @return vector of resulting tokens.
 */
static inline std::vector<std::string> splitAny(const std::string& str, const std::string& delims)
{
    std::string token;
    std::vector<std::string> tokens;
...
static inline void dropEmpty(Container<std::string, Args...>& tokens)
{
 ...
 */
template <template <typename, typename...> typename Container, typename... Args>
static inline Container<std::string> dropEmptyCopy(Container<std::string, Args...> tokens)
{
    drop_empty(tokens);
    return tokens;
}
...
/**
 * @brief Inplace removal of all duplicate strings in a vector<string> where order is not to be
 ...
 */
template <typename T> static inline void dropDuplicate(std::vector<T>& tokens)
{
 ...
 * @return vector of non-duplicate tokens.
 */
template <typename T> static inline std::vector<T> dropDuplicateCopy(std::vector<T> tokens)
{
    std::sort(std::execution::par_unseq, tokens.begin(), tokens.end());
    auto end_unique = std::unique(tokens.begin(), tokens.end());
    tokens.erase(end_unique, tokens.end());
 ...
 */
template <typename T> static inline void sortAscending(std::vector<T>& strs)
{
 ...
 * @param strs - std::vector<std::string> to be checked.
 */
template <typename T> static inline void sortDescending(std::vector<T>& strs)
{
 ...
 */
template <typename Container> static inline void reverseInplace(Container& strs)
{
 ...
 * @param strs - container to be checked.
 */
template <typename Container> static inline Container reverseCopy(Container strs)
{
    std::reverse(strs.begin(), strs.end());
    return strs;
}
