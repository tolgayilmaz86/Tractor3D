      ...
      _listenerItr(nullptr)
{
    ...
    assert(_animation);
    assert(0 <= startTime && startTime <= _animation->_duration && 0 <= endTime
...
void AnimationClip::crossFade(AnimationClip* clip, unsigned long duration)
{
    ...
    if (!isClipStateBitSet(CLIP_IS_PLAYING_BIT)) play();
...
void AnimationClip::removeBeginListener(AnimationClip::Listener* listener)
{
    if (_beginListeners)
    {
        ...
        std::vector<Listener*>::iterator iter =
            std::find(_beginListeners->begin(), _beginListeners->end(), listener);
...
void AnimationClip::removeEndListener(AnimationClip::Listener* listener)
{
    if (_endListeners)
    {
        assert(listener);
        std::vector<Listener*>::iterator iter =
            std::find(_endListeners->begin(), _endListeners->end(), listener);
...
bool AnimationClip::update(float elapsedTime)
{
    ...
    if (isClipStateBitSet(CLIP_IS_MARKED_FOR_REMOVAL_BIT))
    {
        // If the marked for removal bit is set, it means stop() was called on the AnimationClip at
        // some point after the last update call. Reset the flag, and return true so the
        // AnimationClip is removed from the running clips on the AnimationController.
        onEnd();
        return true;
    }
    ...
    if (_repeatCount != REPEAT_INDEFINITE
        && ((_speed >= 0.0f && _elapsedTime >= _activeDuration)
            || (_speed <= 0.0f && _elapsedTime <= 0.0f)))
    {
    ...
    if (_loopBlendTime == 0.0f) percentComplete = MATH_CLAMP(percentComplete, 0.0f, 1.0f);
    ...
    if (isClipStateBitSet(CLIP_IS_FADING_OUT_BIT))
    {
        ...
        if (isClipStateBitSet(
                CLIP_IS_FADING_OUT_STARTED_BIT)) // Calculate elapsed time since the fade out begin.
        {
            assert(_crossFadeToClip);
            _crossFadeOutElapsed =
                (Game::getGameTime() - _crossFadeToClip->_timeStarted) * fabs(_speed);
        ...
        if (_crossFadeOutElapsed < _crossFadeOutDuration)
        {
            ...
            float tempBlendWeight =
                ((float)_crossFadeOutDuration - _crossFadeOutElapsed) / (float)_crossFadeOutDuration;
    ...
    for (size_t i = 0; i < channelCount; i++)
    {
        ...
        assert(channel->getCurve());
        channel->getCurve()->evaluate(percentComplete,
                                      percentageStart,
                                      percentageEnd,
                                      percentageBlend,
                                      value->_value);
