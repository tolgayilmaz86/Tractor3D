...
Animation* AnimationTarget::createAnimation(const std::string& id, Properties* animationProperties)
{
    ...
    for (size_t i = 0; i < keyCount; i++)
    {
        ...
        {
            keyTimes[i] = std::strtoul(static_cast<std::string>(keyTimesStr)
                                           .substr(startOffset, endOffset - startOffset)
                                           .c_str(),
                                       nullptr,
                                       0);
        }
        ...
        {
            keyTimes[i] =
                std::strtoul(static_cast<std::string>(keyTimesStr)
                                 .substr(startOffset, static_cast<std::string>(keyTimesStr).length())
                                 .c_str(),
                             nullptr,
                             0);
        }
    ...
    for (size_t i = 0; i < components; i++)
    {
        ...
        if (endOffset != std::string::npos)
        {
            keyValues[i] = std::atof(static_cast<std::string>(keyValuesStr)
                                         .substr(startOffset, endOffset - startOffset)
                                         .c_str());
        }
        else
        {
            keyValues[i] =
                std::atof(static_cast<std::string>(keyValuesStr)
                              .substr(startOffset, static_cast<std::string>(keyValuesStr).length())
                              .c_str());
        }
        startOffset = endOffset + 1;
    }
    ...
    if (!keyInStr.empty())
    {
        ...
        for (size_t i = 0; i < components; i++)
        {
            ...
            if (endOffset != std::string::npos)
            {
                keyIn[i] = std::atof(static_cast<std::string>(keyInStr)
                                         .substr(startOffset, endOffset - startOffset)
                                         .c_str());
            }
            else
            {
                keyIn[i] =
                    std::atof(static_cast<std::string>(keyInStr)
                                  .substr(startOffset, static_cast<std::string>(keyInStr).length())
                                  .c_str());
            }
            startOffset = endOffset + 1;
        }
    ...
    if (!keyOutStr.empty())
    {
        ...
        for (size_t i = 0; i < components; i++)
        {
            ...
            if (endOffset != std::string::npos)
            {
                keyOut[i] = std::atof(static_cast<std::string>(keyOutStr)
                                          .substr(startOffset, endOffset - startOffset)
                                          .c_str());
            }
            else
            {
                keyOut[i] =
                    std::atof(static_cast<std::string>(keyOutStr)
                                  .substr(startOffset, static_cast<std::string>(keyOutStr).length())
                                  .c_str());
            }
            startOffset = endOffset + 1;
        }
...
void AnimationTarget::removeChannel(Animation::Channel* channel)
{
    if (_animationChannels)
    {
        auto itr = std::remove_if(_animationChannels->begin(),
                                  _animationChannels->end(),
                                  [channel](Animation::Channel* temp) { return temp == channel; });
