...
bool AudioBuffer::streamData(ALuint buffer, bool looped)
{
    ...
    if (_streamStateWav.get())
    {
        ...
        {
            if (looped) _fileStream->seek(_streamStateWav->dataStart, SEEK_SET);
        }
        if (bytesRead > 0)
            AL_CHECK(alBufferData(buffer,
                                  _streamStateWav->format,
                                  buffers,
                                  bytesRead,
                                  _streamStateWav->frequency));
    ...
    else if (_streamStateOgg.get())
    {
        ...
        {
            result = ov_read(&_streamStateOgg->oggFile,
                             buffers + bytesRead,
                             STREAMING_BUFFER_SIZE - bytesRead,
                             0,
                             2,
                             1,
                             &section);
            ...
            {
                if (looped) ov_pcm_seek(&_streamStateOgg->oggFile, _streamStateOgg->dataStart);
                break;
            }
        }
        ...
        if (bytesRead > 0)
            AL_CHECK(alBufferData(buffer,
                                  _streamStateOgg->format,
                                  buffers,
                                  bytesRead,
                                  _streamStateOgg->frequency));
