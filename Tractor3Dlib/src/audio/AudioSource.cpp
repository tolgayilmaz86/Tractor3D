...
AudioSource* AudioSource::create(Properties* properties)
{
    ...
    {
        GP_ERROR("Failed to load audio source from properties object: must be non-null object and "
                 "have namespace equal to 'audio'.");
        return nullptr;
    }
...
bool AudioSource::streamDataIfNeeded()
{
    ...
    int buffersNeeded =
        std::min<int>(_buffer->_buffersNeededCount, AudioBuffer::STREAMING_BUFFER_QUEUE_SIZE);
    if (queuedBuffers < buffersNeeded)
    {
        while (queuedBuffers < buffersNeeded)
        {
            if (!_buffer->streamData(_buffer->_alBufferQueue[queuedBuffers], _looped)) return false;
    ...
    else
    {
        ...
        while (processedBuffers-- > 0)
        {
            ...
            if (!_buffer->streamData(bufferID, _looped)) return false;
