...
#define GP_REG_GAME_SCRIPT_CB(e)                                                                   \
    if (sns->exists(#e))                                                                           \
    _scriptTarget->addScriptCallback(GP_GET_SCRIPT_EVENT(GameScriptTarget, e),                     \
                                     sns->getString(#e).c_str())
...
void Game::frame()
{
    ...
    if (_state == Game::RUNNING)
    {
        ...
        // Run script update.
        if (_scriptTarget)
            _scriptTarget->fireScriptEvent<void>(GP_GET_SCRIPT_EVENT(GameScriptTarget, update),
                                                 elapsedTime);
        ...
        // Run script render.
        if (_scriptTarget)
            _scriptTarget->fireScriptEvent<void>(GP_GET_SCRIPT_EVENT(GameScriptTarget, render),
                                                 elapsedTime);
...
void Game::clear(ClearFlags flags, const Vector4& clearColor, float clearDepth, int clearStencil)
{
    ...
    {
        if (clearColor.x != _clearColor.x || clearColor.y != _clearColor.y
            || clearColor.z != _clearColor.z || clearColor.w != _clearColor.w)
        {
...
void Game::touchEventInternal(Touch::TouchEvent evt, int x, int y, unsigned int contactIndex)
{
    ...
    if (_scriptTarget)
        _scriptTarget->fireScriptEvent<void>(GP_GET_SCRIPT_EVENT(GameScriptTarget, touchEvent),
                                             evt,
                                             x,
                                             y,
                                             contactIndex);
...
bool Game::mouseEventInternal(Mouse::MouseEvent evt, int x, int y, int wheelDelta)
{
    ...
    if (_scriptTarget)
        return _scriptTarget->fireScriptEvent<bool>(GP_GET_SCRIPT_EVENT(GameScriptTarget, mouseEvent),
                                                    evt,
                                                    x,
                                                    y,
                                                    wheelDelta);
...
void Game::resizeEventInternal(unsigned int width, unsigned int height)
{
    ...
    if (_width != width || _height != height)
    {
        ...
        if (_scriptTarget)
            _scriptTarget->fireScriptEvent<void>(GP_GET_SCRIPT_EVENT(GameScriptTarget, resizeEvent),
                                                 width,
                                                 height);
    }
}
...
void Game::gestureSwipeEventInternal(int x, int y, int direction)
{
    ...
    if (_scriptTarget)
        _scriptTarget->fireScriptEvent<void>(GP_GET_SCRIPT_EVENT(GameScriptTarget, gestureSwipeEvent),
                                             x,
                                             y,
                                             direction);
}
...
void Game::gesturePinchEventInternal(int x, int y, float scale)
{
    ...
    if (_scriptTarget)
        _scriptTarget->fireScriptEvent<void>(GP_GET_SCRIPT_EVENT(GameScriptTarget, gesturePinchEvent),
                                             x,
                                             y,
                                             scale);
}
...
void Game::gestureTapEventInternal(int x, int y)
{
    gestureTapEvent(x, y);
    if (_scriptTarget)
        _scriptTarget->fireScriptEvent<void>(GP_GET_SCRIPT_EVENT(GameScriptTarget, gestureTapEvent),
                                             x,
                                             y);
}
...
void Game::gestureLongTapEventInternal(int x, int y, float duration)
{
    ...
    if (_scriptTarget)
        _scriptTarget->fireScriptEvent<void>(GP_GET_SCRIPT_EVENT(GameScriptTarget,
                                                                 gestureLongTapevent),
                                             x,
                                             y,
                                             duration);
...
void Game::gestureDragEventInternal(int x, int y)
{
    ...
    if (_scriptTarget)
        _scriptTarget->fireScriptEvent<void>(GP_GET_SCRIPT_EVENT(GameScriptTarget, gestureDragEvent),
                                             x,
                                             y);
}
...
void Game::gestureDropEventInternal(int x, int y)
{
    ...
    if (_scriptTarget)
        _scriptTarget->fireScriptEvent<void>(GP_GET_SCRIPT_EVENT(GameScriptTarget, gestureDropEvent),
                                             x,
                                             y);
}
...
void Game::gamepadEventInternal(Gamepad::GamepadEvent evt, Gamepad* gamepad)
{
    ...
    if (_scriptTarget)
        _scriptTarget->fireScriptEvent<void>(GP_GET_SCRIPT_EVENT(GameScriptTarget, gamepadEvent),
                                             evt,
                                             gamepad);
}
