...
static tractor::Keyboard::Key getKey(WPARAM win32KeyCode, bool shiftDown)
{
    switch (win32KeyCode)
    {
        ...
        case VK_OEM_4:
            return shiftDown ? tractor::Keyboard::KEY_LEFT_BRACE
                             : tractor::Keyboard::KEY_LEFT_BRACKET;
        case VK_OEM_5:
            return shiftDown ? tractor::Keyboard::KEY_BAR : tractor::Keyboard::KEY_BACK_SLASH;
        case VK_OEM_6:
            return shiftDown ? tractor::Keyboard::KEY_RIGHT_BRACE
                             : tractor::Keyboard::KEY_RIGHT_BRACKET;
        case VK_OEM_7:
    ...
    switch (msg)
    {
        ...
        case WM_MOUSEWHEEL:
            ...
            tractor::Platform::mouseEventInternal(tractor::Mouse::MOUSE_WHEEL,
                                                  point.x,
                                                  point.y,
                                                  GET_WHEEL_DELTA_WPARAM(wParam) / 120);
            break;
...
extern int strcmpnocase(const char* s1, const char* s2) { return _strcmpi(s1, s2); }
...
Platform* Platform::create(Game* game)
{
    ...
    if (game->getConfig())
    {
        ...
        if (config)
        {
            ...
            if (width == 0 && height == 0 && params.fullscreen) getDesktopResolution(width, height);
    ...
    if (!initializeGL(&params)) goto error;
    ...
    for (DWORD i = 0; i < XUSER_MAX_COUNT; i++)
    {
        if (XInputGetState(i, &__xInputState) == NO_ERROR)
        {
            if (!__connectedXInput[i])
            {
                ...
                Platform::gamepadEventConnectedInternal(i,
                                                        XINPUT_BUTTON_COUNT,
...
int Platform::enterMessagePump()
{
    ...
    if (_game->getState() != Game::RUNNING) _game->run();
    ...
    while (true)
    {
        ...
        if (_game->getState() == Game::UNINITIALIZED) break;
    }
    return 0;
}
...
void Platform::pollGamepadState(Gamepad* gamepad)
{
    ...
    if (XInputGetState(gamepad->_handle, &__xInputState) == NO_ERROR)
    {
        ...
        for (i = 0; i < gamepad->_joystickCount; ++i)
        {
            ...
            {
                case 0:
                    x = normalizeXInputJoystickAxis(__xInputState.Gamepad.sThumbLX,
                                                    XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE);
                    y = normalizeXInputJoystickAxis(__xInputState.Gamepad.sThumbLY,
                                                    XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE);
                    break;
                case 1:
                    x = normalizeXInputJoystickAxis(__xInputState.Gamepad.sThumbRX,
                                                    XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE);
                    y = normalizeXInputJoystickAxis(__xInputState.Gamepad.sThumbRY,
                                                    XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE);
                    break;
            }
