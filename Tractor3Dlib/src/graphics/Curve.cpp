// Purposely not including Base.h here, or any other gameplay dependencies, so it can be reused
// between gameplay and gameplay-encoder.
#include "pch.h"
...
void Curve::getPointValues(unsigned int index, float* value, float* inValue, float* outValue) const
{
    ...
    if (value) memcpy(value, _points[index].value, _componentSize);

    if (inValue) memcpy(inValue, _points[index].inValue, _componentSize);
...
void Curve::setPoint(unsigned int index,
                     ...
                     float* outValue)
{
    assert(index < _pointCount && time >= 0.0f && time <= 1.0f
           && !(_pointCount > 1 && index == 0 && time != 0.0f)
           && !(_pointCount != 1 && index == _pointCount - 1 && time != 1.0f));

    _points[index].time = time;
    _points[index].type = type;

    if (value) memcpy(_points[index].value, value, _componentSize);

    if (inValue) memcpy(_points[index].inValue, inValue, _componentSize);
    ...
    switch (from->type)
    {
        ...
        case ELASTIC_OUT_IN:
        {
            if (t != 0.0f && t != 1.0f)
            {
                ...
                {
                    t = 0.5f * (exp((10 * (t - 2))) * sin(((t - 0.1125f) * (MATH_PIX2) / 0.45f)))
                        + 0.5f;
                }
            }
            break;
...
void Curve::setQuaternionOffset(unsigned int offset)
{
    ...
    if (!_quaternionOffset) _quaternionOffset = new unsigned int[1];
    ...
    if (!_quaternionOffset)
    {
        for (size_t i = 0; i < _componentCount; i++)
        {
            if (fromValue[i] == toValue[i])
                ...
                dst[i] =
                    bezier(eq1, eq2, eq3, eq4, fromValue[i], outValue[i], toValue[i], inValue[i]);
        }
    }
    else
    {
    ...
    if (!_quaternionOffset)
    {
        for (size_t i = 0; i < _componentCount; i++)
        {
            if (fromValue[i] == toValue[i])
                ...
                dst[i] =
                    hermite(h00, h01, h10, h11, fromValue[i], outValue[i], toValue[i], inValue[i]);
        }
    }
    else
    {
    ...
    if (!_quaternionOffset)
    {
        for (size_t i = 0; i < _componentCount; i++)
        {
            ...
            else
            {
                ...
                {
                    outValue = (toValue[i] - (from - 1)->value[i])
                               * ((from->time - (from - 1)->time) / (to->time - (from - 1)->time));
                }
                ...
                {
                    inValue = ((to + 1)->value[i] - fromValue[i])
                              * ((to->time - from->time) / ((to + 1)->time - from->time));
                }

                dst[i] =
                    hermiteSmooth(h00, h01, h10, h11, fromValue[i], outValue, toValue[i], inValue);
            }
        }
    }
    else
