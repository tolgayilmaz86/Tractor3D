...
namespace tractor
{
	...
	Effect* Effect::createFromFile(const std::string& vshPath, const std::string& fshPath, const std::string& defines)
	{
	...
	Effect* Effect::createFromSource(const std::string& vshSource, const std::string& fshSource, const std::string& defines)
	{
	...
	Effect* Effect::createFromSource(const std::string& vshPath, const std::string& vshSource, const std::string& fshPath, const std::string& fshSource, const std::string& defines)
	{
		...
		if (success != GL_TRUE)
		{
			...
			GP_ERROR("Compile failed for vertex shader '%s' with error '%s'.", vshPath == EMPTY_STRING ? vshSource : vshPath, infoLog == EMPTY_STRING ? "" : infoLog);
		...
		if (success != GL_TRUE)
		{
			...
			GP_ERROR("Compile failed for fragment shader (%s): %s", fshPath == EMPTY_STRING ? fshSource : fshPath, infoLog == EMPTY_STRING ? "" : infoLog);
			SAFE_DELETE_ARRAY(infoLog);
		...
		if (success != GL_TRUE)
		{
			...
			GP_ERROR("Linking program failed (%s,%s): %s", vshPath == EMPTY_STRING ? "nullptr" : vshPath, fshPath == EMPTY_STRING ? "nullptr" : fshPath, infoLog == EMPTY_STRING ? "" : infoLog);
			SAFE_DELETE_ARRAY(infoLog);
