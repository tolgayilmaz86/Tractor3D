...
void Plane::intersection(const Plane& p1, const Plane& p2, const Plane& p3, Vector3* point)
{
    ...
    if (fabs(det) <= MATH_EPSILON) return;
...
float Plane::intersects(const Frustum& frustum) const
{
    ...
    if (d > 0.0f)
    {
        if (distance(corners[1]) <= 0.0f || distance(corners[2]) <= 0.0f || distance(corners[3]) <= 0.0f
            || distance(corners[4]) <= 0.0f || distance(corners[5]) <= 0.0f
            || distance(corners[6]) <= 0.0f || distance(corners[7]) <= 0.0f)
        {
...
float Plane::intersects(const Plane& plane) const
{
    ...
    Vector3 point(plane._normal.x * -plane._distance,
                  plane._normal.y * -plane._distance,
                  plane._normal.z * -plane._distance);
...
float Plane::intersects(const Ray& ray) const
{
    ...
    else
    {
        ...
        if (_normal.x * rayDirection.x + _normal.y * rayDirection.y + _normal.z * rayDirection.z
            > 0.0f)
        {
            if (d < 0.0f)
            {
...
void Plane::transform(const Matrix& matrix)
{
    ...
    {
        // Treat the plane as a four-tuple and multiply by the inverse transpose of the matrix to
        // get the transformed plane. Then we normalize the plane by dividing both the normal and
        // the distance by the length of the normal.
        ...
        float nz = _normal.x * inverted.m[8] + _normal.y * inverted.m[9]
                   + _normal.z * inverted.m[10] + _distance * inverted.m[11];
        float d = _normal.x * inverted.m[12] + _normal.y * inverted.m[13]
                  + _normal.z * inverted.m[14] + _distance * inverted.m[15];
...
void Plane::normalize()
{
    ...
    float normalizeFactor =
        1.0f / sqrt(_normal.x * _normal.x + _normal.y * _normal.y + _normal.z * _normal.z);
