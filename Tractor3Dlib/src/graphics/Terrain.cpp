...
#define TERRAIN_MATERIAL "res/materials/terrain.material"
  ...
  // does not have an explicitly specified patch size.
	...
	Terrain* Terrain::create(const std::string& path)
	{
	...
	Terrain* Terrain::create(const std::string& path, Properties* properties)
	{
		...
		// Create terrain
		Terrain* terrain = create(heightfield, scale, (unsigned int)patchSize, (unsigned int)detailLevels, skirtScale, normalMap, materialPath, pTerrain);
