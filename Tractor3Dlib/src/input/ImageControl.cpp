...
ImageControl::ImageControl()
    : _srcRegion(Rectangle::empty()), _dstRegion(Rectangle::empty()), _batch(nullptr), _tw(0.0f),
      _th(0.0f), _uvs(Theme::UVs::full())
{
}
...
{
    static const std::string TYPE_NAME = "ImageControl";
    return TYPE_NAME;
}
...
unsigned int ImageControl::drawImages(Form* form, const Rectangle& clip)
{
    ...
    {
        _batch->draw(_viewportBounds.x + _dstRegion.x,
                     _viewportBounds.y + _dstRegion.y,
                     _dstRegion.width,
                     _dstRegion.height,
                     _uvs.u1,
                     _uvs.v1,
