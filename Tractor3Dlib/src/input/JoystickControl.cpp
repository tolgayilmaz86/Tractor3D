                                                   ...
                                                   bool* isHeightPercentage) const
{
    if (isWidthPercentage)
    {
        *isWidthPercentage =
            _innerSizePixels
            && (_innerRegionCoord && _innerRegionCoordBoundsBits & BOUNDS_WIDTH_PERCENTAGE_BIT
                || Control::isXPercentage());
    }
    ...
    if (isHeightPercentage)
    {
        *isHeightPercentage =
            _innerSizePixels
            && (_innerRegionCoord && _innerRegionCoordBoundsBits & BOUNDS_HEIGHT_PERCENTAGE_BIT
                || Control::isYPercentage());
    }
                                                   ...
                                                   bool* isHeightPercentage) const
{
    if (isWidthPercentage)
    {
        *isWidthPercentage =
            _outerSizePixels
            && (_outerRegionCoord && _outerRegionCoordBoundsBits & BOUNDS_WIDTH_PERCENTAGE_BIT
                || Control::isXPercentage());
    }
    ...
    if (isHeightPercentage)
    {
        *isHeightPercentage =
            _outerSizePixels
            && (_outerRegionCoord && _outerRegionCoordBoundsBits & BOUNDS_HEIGHT_PERCENTAGE_BIT
                || Control::isYPercentage());
    }
                                 ...
                                 Properties* properties)
{
    ...
    const std::string radiusId{ "radius" };
    ...
    {
        const auto& radiusStr = properties->getString(radiusId);
        bool isPercentage = false;
        _radiusCoord = parseCoord(radiusStr, &isPercentage);
        setBoundsBit(isPercentage, _boundsBits, BOUNDS_RADIUS_PERCENTAGE_BIT);
    }

    const std::string relativeId{ "relative" };
...
{
    Vector2 size;
    size.x = regionBoundsBits & BOUNDS_WIDTH_PERCENTAGE_BIT ? _absoluteBounds.width * region.x
                                                            : region.x;
    size.y = regionBoundsBits & BOUNDS_HEIGHT_PERCENTAGE_BIT ? _absoluteBounds.height * region.y
                                                             : region.y;
    return size;
}
...
Vector2 JoystickControl::getPixelSize(const Theme::ThemeImage* image) const
{
    ...
    rect.height =
        isHeightPercentage() ? (_absoluteBounds.height / rect.height) * rect.height : rect.height;
...
void JoystickControl::updateAbsoluteSizes()
{
    ...
    if (innerRegionDefined)
    {
        ...
        *_innerSizePixels = _innerRegionCoord
                                ? getPixelSize(*_innerRegionCoord, _innerRegionCoordBoundsBits)
                                : getPixelSize(innerImage);
    ...
    if (outerRegionDefined)
    {
        ...
        *_outerSizePixels = _outerRegionCoord
                                ? getPixelSize(*_outerRegionCoord, _outerRegionCoordBoundsBits)
                                : getPixelSize(outerImage);
        _screenRegionPixels.width = _outerSizePixels->x > _screenRegionPixels.width
                                        ? _outerSizePixels->x
                                        : _screenRegionPixels.width;
        _screenRegionPixels.height = _outerSizePixels->y > _screenRegionPixels.height
                                         ? _outerSizePixels->y
                                         : _screenRegionPixels.height;
    }

    _radiusPixels =
        std::max(1.0f,
                 _boundsBits & BOUNDS_RADIUS_PERCENTAGE_BIT
                     ? std::min(_screenRegionPixels.width, _screenRegionPixels.height) * _radiusCoord
                     : _radiusCoord);
    ...
    switch (evt)
    {
        case Touch::TOUCH_PRESS:
        {
            if (_contactIndex == INVALID_CONTACT_INDEX)
            {
                ...
                Vector2 value;
                if ((fabs(_displacement.x) > _radiusPixels)
                    || (fabs(_displacement.y) > _radiusPixels))
                {
        ...
        case Touch::TOUCH_MOVE:
        {
            ...
            {
                float dx = x - ((_relative) ? _screenRegionPixels.x - _bounds.x : 0.0f)
                           - _screenRegionPixels.width * 0.5f;
                float dy = -(y - ((_relative) ? _screenRegionPixels.y - _bounds.y : 0.0f)
                             - _screenRegionPixels.height * 0.5f);
...
unsigned int JoystickControl::drawImages(Form* form, const Rectangle& clip)
{
    ...
    if ((_outerSizePixels || _innerSizePixels) && !_relative || (_relative && state == ACTIVE))
    {
        if (!_relative)
        {
            _screenRegionPixels.x = _viewportClipBounds.x
                                    + (_viewportClipBounds.width - _screenRegionPixels.width) / 2.0f;
            _screenRegionPixels.y =
                _viewportClipBounds.y
                + (_viewportClipBounds.height - _screenRegionPixels.height) / 2.0f;
        }
