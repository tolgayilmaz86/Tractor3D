...
bool PhysicsCollisionObject::collidesWith(PhysicsCollisionObject* object) const
{
    ...
    Game::getInstance()->getPhysicsController()->_world->contactPairTest(getCollisionObject(),
...
bool PhysicsCollisionObject::CollisionPair::operator<(const CollisionPair& collisionPair) const
{
    ...
    if ((objectA == collisionPair.objectA && objectB == collisionPair.objectB)
        || (objectA == collisionPair.objectB && objectB == collisionPair.objectA))
        return false;

    // We choose to compare based on objectA arbitrarily.
    if (objectA < collisionPair.objectA) return true;

    if (objectA == collisionPair.objectA) return objectB < collisionPair.objectB;
...
void PhysicsCollisionObject::PhysicsMotionState::getWorldTransform(btTransform& transform) const
{
    ...
    if (_collisionObject->isKinematic()) updateTransformFromNode();
...
void PhysicsCollisionObject::PhysicsMotionState::updateTransformFromNode() const
{
    ...
    if (!_centerOfMassOffset.getOrigin().isZero())
    {
        ...
        btTransform offset =
            btTransform(BQ(rotation), btVector3(0.0f, 0.0f, 0.0f)) * _centerOfMassOffset.inverse();
    ...
    const Vector3& contactPointB)
{
    Game::getInstance()
        ->getScriptController()
        ->executeFunction<void>(function.c_str(),
                                "[PhysicsCollisionObject::CollisionListener::EventType]<"
                                "PhysicsCollisionObject::CollisionPair><Vector3><Vector3>",
                                nullptr,
                                type,
                                &collisionPair,
                                &contactPointA,
