...
{
    checkConstraintRigidBodies(a, b);
    PhysicsGenericConstraint* constraint = new PhysicsGenericConstraint(a,
                                                                        rotationOffsetA,
                                                                        translationOffsetA,
                                                                        b,
                                                                        rotationOffsetB,
...
{
    checkConstraintRigidBodies(a, b);
    PhysicsHingeConstraint* constraint = new PhysicsHingeConstraint(a,
                                                                    rotationOffsetA,
                                                                    translationOffsetA,
                                                                    b,
                                                                    rotationOffsetB,
                                                                   ...
                                                                   const Vector3& translationOffsetB)
{
    ...
    PhysicsSocketConstraint* constraint =
        new PhysicsSocketConstraint(a, translationOffsetA, b, translationOffsetB);
    addConstraint(a, b, constraint);
    return constraint;
}
...
{
    checkConstraintRigidBodies(a, b);
    PhysicsSpringConstraint* constraint = new PhysicsSpringConstraint(a,
                                                                      rotationOffsetA,
                                                                      translationOffsetA,
                                                                      b,
                                                                      rotationOffsetB,
                                ...
                                PhysicsController::HitFilter* filter)
{
    class RayTestCallback : public btCollisionWorld::ClosestRayResultCallback
    {
      ...
      public:
        RayTestCallback(const btVector3& rayFromWorld,
        ...
        virtual bool needsCollision(btBroadphaseProxy* proxy0) const
        {
            ...
            PhysicsCollisionObject* object =
                reinterpret_cast<PhysicsCollisionObject*>(co->getUserPointer());
        ...
        btScalar addSingleResult(btCollisionWorld::LocalRayResult& rayResult, bool normalInWorldSpace)
        {
            ...
            if (object == nullptr) return 1.0f; // ignore
            ...
            if (filter && !filter->hit(hitResult)) return 1.0f; // process next collision
                                  ...
                                  PhysicsController::HitFilter* filter)
{
    class SweepTestCallback : public btCollisionWorld::ClosestConvexResultCallback
    {
      ...
      public:
        ...
        virtual bool needsCollision(btBroadphaseProxy* proxy0) const
        {
            ...
            PhysicsCollisionObject* object =
                reinterpret_cast<PhysicsCollisionObject*>(co->getUserPointer());
            if (object == nullptr || object == me) return false;
                                 ...
                                 bool normalInWorldSpace)
        {
            ...
            if (filter && !filter->hit(hitResult)) return 1.0f;
    ...
    if (type != PhysicsCollisionShape::SHAPE_BOX && type != PhysicsCollisionShape::SHAPE_SPHERE
    ...
    case PhysicsCollisionObject::CHARACTER:
        static_cast<PhysicsGhostObject*>(object)->_ghostObject->convexSweepTest(static_cast<btConvexShape*>(shape->getShape()),
    start, end, callback, _world->getDispatchInfo().m_allowedCcdPenetration); break;

    default:
        _world->convexSweepTest(static_cast<btConvexShape*>(shape->getShape()), start, end,
    callback, _world->getDispatchInfo().m_allowedCcdPenetration); break;
    }*/

    assert(_world);
    _world->convexSweepTest(static_cast<btConvexShape*>(shape->getShape()),
                            start,
                            end,
                            callback,
                            _world->getDispatchInfo().m_allowedCcdPenetration);
                                                               ...
                                                               int indexB)
{
    ...
    else
    {
        ...
        if (_pc->_collisionStatus.count(p1) > 0)
        {
            ...
            for (const auto& listener : ci._listeners)
            {
                assert(listener);
                collisionInfo->_listeners.push_back(listener);
            }
        }
    ...
    {
        for (const auto& listener : collisionInfo->_listeners)
        {
            assert(listener);
            if ((collisionInfo->_status & REMOVE) == 0)
            {
                listener->collisionEvent(PhysicsCollisionObject::CollisionListener::COLLIDING,
                                         pair,
                                         Vector3(cp.getPositionWorldOnA().x(),
                                                 cp.getPositionWorldOnA().y(),
                                                 cp.getPositionWorldOnA().z()),
                                         Vector3(cp.getPositionWorldOnB().x(),
...
void PhysicsController::initialize()
{
    ...
    // Create the world.
    _world = bullet_new<btDiscreteDynamicsWorld>(_dispatcher,
...
void PhysicsController::update(float elapsedTime)
{
    ...
    if (_listeners || hasScriptListener(GP_GET_SCRIPT_EVENT(PhysicsController, statusEvent)))
    {
        ...
        else
        {
            ...
            if (allInactive) _status = Listener::DEACTIVATED;
        }
    ...
    std::map<PhysicsCollisionObject::CollisionPair, CollisionInfo>::iterator iter =
        _collisionStatus.begin();
        ...
        if ((iter->second._status & REMOVE) != 0)
        {
            if ((iter->second._status & COLLISION) != 0 && iter->first.objectB)
            {
                ...
                for (size_t i = 0; i < size; i++)
                {
                    ...
                    iter->second._listeners[i]
                        ->collisionEvent(PhysicsCollisionObject::CollisionListener::NOT_COLLIDING,
                                         cp);
                }
            }
    ...
    for (; iter != _collisionStatus.end(); iter++)
    {
        ...
        if ((iter->second._status & REGISTERED) != 0 && (iter->second._status & REMOVE) == 0)
        {
            if (iter->first.objectB)
                _world->contactPairTest(iter->first.objectA->getCollisionObject(),
                                        iter->first.objectB->getCollisionObject(),
                                        *_collisionCallback);
            else
    ...
    switch (shape.type)
    {
        case PhysicsCollisionShape::SHAPE_BOX:
        {
            if (shape.isExplicit)
            {
                ...
                {
                    computeCenterOfMass(Vector3(shape.data.box.center),
                                        Vector3::one(),
                                        centerOfMassOffset);
                }
                else
                {
                    ...
                    computeCenterOfMass(box.getCenter() + Vector3(shape.data.box.center),
                                        scale,
                                        centerOfMassOffset);
                }
            }
            else
            {
                ...
                BoundingBox box;
                getBoundingBox(node, &box);
                collisionShape = createBox(Vector3(std::fabs(box.max.x - box.min.x),
                                                   std::fabs(box.max.y - box.min.y),
                                                   std::fabs(box.max.z - box.min.z)),
                                           scale);
        ...
        case PhysicsCollisionShape::SHAPE_SPHERE:
        {
            if (shape.isExplicit)
            {
                ...
                if (shape.centerAbsolute)
                {
                    computeCenterOfMass(Vector3(shape.data.sphere.center),
                                        Vector3::one(),
                                        centerOfMassOffset);
                }
                else
                {
                    ...
                    computeCenterOfMass(sphere.center + Vector3(shape.data.sphere.center),
                                        scale,
                                        centerOfMassOffset);
                }
            }
        ...
        case PhysicsCollisionShape::SHAPE_CAPSULE:
        {
            if (shape.isExplicit)
            {
                ...
                collisionShape = createCapsule(shape.data.capsule.radius,
                                               shape.data.capsule.height,
                                               Vector3::one());

                if (shape.centerAbsolute)
                {
                    computeCenterOfMass(Vector3(shape.data.capsule.center),
                                        Vector3::one(),
                                        centerOfMassOffset);
                }
                else
                {
                    BoundingBox box;
                    getBoundingBox(node, &box);
                    computeCenterOfMass(box.getCenter() + Vector3(shape.data.capsule.center),
                                        scale,
                                        centerOfMassOffset);
                }
            }
            else
            {
                ...
                getBoundingBox(node, &box);
                float radius =
                    std::max((box.max.x - box.min.x) * 0.5f, (box.max.z - box.min.z) * 0.5f);
        ...
        case PhysicsCollisionShape::SHAPE_HEIGHTFIELD:
        {
            ...
            else
            {
                ...
                if (dynamic_cast<Terrain*>(node->getDrawable()) == nullptr)
                    GP_ERROR("Empty heightfield collision shapes can only be used on nodes that "
                             "have an attached Terrain.");
                else
                    collisionShape =
                        createHeightfield(node,
                                          dynamic_cast<Terrain*>(node->getDrawable())->_heightfield,
                                          centerOfMassOffset);
            }
        }
        break;
...
PhysicsCollisionShape* PhysicsController::createSphere(float radius, const Vector3& scale)
{
    ...
    if (uniformScale < scale.y) uniformScale = scale.y;
    if (uniformScale < scale.z) uniformScale = scale.z;
                                                            ...
                                                            Vector3* centerOfMassOffset)
{
    ...
    // Compute initial center of mass offset necessary to move the height from its position in
    // bullet physics (always centered around origin) to its intended location.
    ...
    btHeightfieldTerrainShape* terrainShape =
        bullet_new<btHeightfieldTerrainShape>(heightfield->getColumnCount(),
...
PhysicsCollisionShape* PhysicsController::createMesh(Mesh* mesh, const Vector3& scale, bool dynamic)
{
    ...
    if (!dynamic)
    {
        ...
        // Dynamic meshes are approximated with a btConvexHullShape (convex wrapper on cloud of
        // vertices) and therefore can support any primitive type.
        bool triMesh = true;
        ...
        if (!triMesh)
        {
            GP_ERROR("Mesh rigid bodies are currently only supported on meshes with TRIANGLES "
                     "primitive type.");
            return nullptr;
        }
    }
    ...
    if (dynamic)
    {
        ...
        btConvexHullShape* originalConvexShape =
            bullet_new<btConvexHullShape>(shapeMeshData->vertexData,
                                          data->vertexCount,
                                          sizeof(float) * 3);
    ...
    else
    {
        ...
        if (partCount > 0)
        {
            ...
            for (size_t i = 0; i < partCount; i++)
            {
                ...
                indexedMesh.m_numTriangles =
                    meshPart->indexCount / 3; // assume TRIANGLES primitive type
...
bool PhysicsController::checkConstraintRigidBodies(PhysicsRigidBody* a, PhysicsRigidBody* b)
{
    ...
    {
        assert(a->_node);
        GP_ERROR("Rigid body '%s' does not support constraints; unexpected behavior may occur.",
                 a->_node->getId());
        return false;
    }
...
PhysicsController::DebugDrawer::DebugDrawer()
    : _mode(btIDebugDraw::DBG_DrawAabb | btIDebugDraw::DBG_DrawConstraintLimits
      ...
      _meshBatch(nullptr), _lineCount(0)
{
    // Vertex shader for drawing colored lines.
    const std::string vs_str{ "uniform mat4 u_viewProjectionMatrix;\n"
