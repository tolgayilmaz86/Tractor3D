      ...
      _constraints(nullptr), _inDestructor(false)
{
    ...
    _collisionShape =
        Game::getInstance()->getPhysicsController()->createShape(node,
                                                                 shape,
                                                                 &centerOfMassOffset,
                                                                 parameters.mass != 0.0f);
    ...
    btRigidBody::btRigidBodyConstructionInfo rbInfo(parameters.mass,
                                                    nullptr,
                                                    _collisionShape->getShape(),
                                                    localInertia);
...
PhysicsRigidBody* PhysicsRigidBody::create(Node* node, Properties* properties, const std::string& nspace)
{
    ...
    {
        GP_ERROR("Failed to load rigid body from properties object: must be non-null object and "
                 "have namespace equal to 'collisionObject'.");
        return nullptr;
    }
    ...
    if (type.empty())
    {
        GP_ERROR("Failed to load physics rigid body from properties object; required attribute "
                 "'type' is missing.");
        return nullptr;
    }
    if (type != nspace)
    {
        GP_ERROR("Failed to load physics rigid body from properties object; attribute 'type' must "
                 "be equal to '%s'.",
                 nspace);
        return nullptr;
    }
    ...
    while (auto property = properties->getNextProperty())
    {
        ...
        else if (name == "anisotropicFriction")
        {
            properties->getVector3(EMPTY_STRING, &parameters.anisotropicFriction);
        }
        ...
        {
            properties->getVector3(EMPTY_STRING, &parameters.angularFactor);
        }
        else if (name == "linearFactor")
        {
            properties->getVector3(EMPTY_STRING, &parameters.linearFactor);
        }
        else
        {
...
void PhysicsRigidBody::transformChanged(Transform* transform, long cookie)
{
    if (getShapeType() == PhysicsCollisionShape::SHAPE_HEIGHTFIELD)
    {
        ...
        _motionState->setCenterOfMassOffset(
            Vector3(0, -(minHeight + (maxHeight - minHeight) * 0.5f) * scale.y, 0));
    }
}
