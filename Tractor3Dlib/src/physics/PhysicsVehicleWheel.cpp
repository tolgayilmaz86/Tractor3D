...
{
    GP_ERROR("Operation not supported (PhysicsVehicleWheel::setEnabled(bool)). Use host vehicle "
             "instead.");
}
...
void PhysicsVehicleWheel::addToVehicle(btRaycastVehicle* vehicle)
{
    ...
    vehicle->addWheel(btVector3(0, 0, 0),
                      btVector3(0, -1, 0),
                      btVector3(-1, 0, 0),
                      0.6f,
                      0.5f,
...
void PhysicsVehicleWheel::transform(Node* node) const
{
    ...
    node->setTranslation(wheelPos
                         + strutLine * (strutLine.dot(_positionDelta) / strutLine.lengthSquared()));
}
...
void PhysicsVehicleWheel::setWheelDirection(const Vector3& wheelDirection)
{
    ...
    _host->_vehicle->getWheelInfo(_indexInHost)
        .m_wheelDirectionCS.setValue(wheelDirection.x, wheelDirection.y, wheelDirection.z);
}
...
void PhysicsVehicleWheel::setWheelAxle(const Vector3& wheelAxle)
{
    ...
    _host->_vehicle->getWheelInfo(_indexInHost)
        .m_wheelAxleCS.setValue(wheelAxle.x, wheelAxle.y, wheelAxle.z);
}
...
void PhysicsVehicleWheel::setStrutConnectionOffset(const Vector3& strutConnectionOffset)
{
    ...
    _host->_vehicle->getWheelInfo(_indexInHost)
        .m_chassisConnectionPointCS.setValue(strutConnectionPoint.x,
