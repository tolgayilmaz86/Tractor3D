...
Camera* Camera::create(Properties* properties)
{
    ...
    switch (type)
    {
        case Camera::PERSPECTIVE:
            ...
            camera = createPerspective(properties->exists("fieldOfView")
                                           ? properties->getFloat("fieldOfView")
                                           : 60.0f,
                                       aspectRatio,
                                       nearPlane,
                                       farPlane);
        ...
        case Camera::ORTHOGRAPHIC:
            ...
            camera =
                createOrthographic(properties->exists("zoomX") ? properties->getFloat("zoomX")
                                                               : Game::getInstance()->getWidth(),
                                   properties->exists("zoomY") ? properties->getFloat("zoomY")
...
void Camera::setNode(Node* node)
{
    if (_node != node)
    {
        ...
        _bits |= CAMERA_DIRTY_VIEW | CAMERA_DIRTY_VIEW_PROJ | CAMERA_DIRTY_INV_VIEW
                 | CAMERA_DIRTY_INV_VIEW_PROJ | CAMERA_DIRTY_BOUNDS;
        cameraChanged();
    }
}
...
const Matrix& Camera::getProjectionMatrix() const
{
    if (!(_bits & CAMERA_CUSTOM_PROJECTION) && (_bits & CAMERA_DIRTY_PROJ))
    {
        ...
        {
            _projection =
                Matrix::createPerspective(_fieldOfView, _aspectRatio, _nearPlane, _farPlane);
        }
        else
        {
                     ...
                     float* depth) const
{
    ...
    Vector4 clipPos;
    getViewProjectionMatrix().transformVector(Vector4(position.x, position.y, position.z, 1.0f),
                                              &clipPos);
...
void Camera::unproject(const Rectangle& viewport, float x, float y, float depth, Vector3* dst) const
{
    ...
    Vector4 screen((x - viewport.x) / viewport.width,
                   ((viewport.height - y) - viewport.y) / viewport.height,
                   depth,
                   1.0f);
...
Camera* Camera::clone(NodeCloneContext& context) const
{
    ...
    {
        cameraClone =
            createOrthographic(getZoomX(), getZoomY(), getAspectRatio(), _nearPlane, _farPlane);
    }
    assert(cameraClone);
...
void Camera::transformChanged(Transform* transform, long cookie)
{
    _bits |= CAMERA_DIRTY_VIEW | CAMERA_DIRTY_INV_VIEW | CAMERA_DIRTY_INV_VIEW_PROJ
             | CAMERA_DIRTY_VIEW_PROJ | CAMERA_DIRTY_BOUNDS;
