...
namespace tractor
{
	...
	FrameBuffer::FrameBuffer(const std::string& id, unsigned int width, unsigned int height, FrameBufferHandle handle)
		: _id(id), _handle(handle), _renderTargets(nullptr), _renderTargetCount(0), _depthStencilTarget(nullptr)
	{
	}
	...
	FrameBuffer* FrameBuffer::create(const std::string& id, unsigned int width, unsigned int height, Texture::Format format)
	{
