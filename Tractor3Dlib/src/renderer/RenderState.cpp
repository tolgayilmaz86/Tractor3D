...
void RenderState::setParameterAutoBinding(const std::string& name, const std::string& autoBinding)
{
    if (autoBinding.empty())
    {
        ...
        if (itr != _autoBindings.end()) _autoBindings.erase(itr);
    }
    else
    {
...
void RenderState::applyAutoBinding(const std::string& uniformName, const std::string& autoBinding)
{
    ...
    static const std::unordered_map<std::string, std::function<void(MaterialParameter*, RenderState*)>>
        autoBindingMap = {
            { "WORLD_MATRIX",
              [](MaterialParameter* param, RenderState* rs)
              { param->bindValue(rs, &RenderState::autoBindingGetWorldMatrix); } },
            { "VIEW_MATRIX",
              [](MaterialParameter* param, RenderState* rs)
              { param->bindValue(rs, &RenderState::autoBindingGetViewMatrix); } },
            { "PROJECTION_MATRIX",
              [](MaterialParameter* param, RenderState* rs)
              { param->bindValue(rs, &RenderState::autoBindingGetProjectionMatrix); } },
            { "WORLD_VIEW_MATRIX",
              [](MaterialParameter* param, RenderState* rs)
              { param->bindValue(rs, &RenderState::autoBindingGetWorldViewMatrix); } },
            { "VIEW_PROJECTION_MATRIX",
              [](MaterialParameter* param, RenderState* rs)
              { param->bindValue(rs, &RenderState::autoBindingGetViewProjectionMatrix); } },
            { "WORLD_VIEW_PROJECTION_MATRIX",
              [](MaterialParameter* param, RenderState* rs)
              { param->bindValue(rs, &RenderState::autoBindingGetWorldViewProjectionMatrix); } },
            { "INVERSE_TRANSPOSE_WORLD_MATRIX",
              [](MaterialParameter* param, RenderState* rs)
              { param->bindValue(rs, &RenderState::autoBindingGetInverseTransposeWorldMatrix); } },
            { "INVERSE_TRANSPOSE_WORLD_VIEW_MATRIX",
              [](MaterialParameter* param, RenderState* rs) {
                  param->bindValue(rs, &RenderState::autoBindingGetInverseTransposeWorldViewMatrix);
              } },
            { "CAMERA_WORLD_POSITION",
              [](MaterialParameter* param, RenderState* rs)
              { param->bindValue(rs, &RenderState::autoBindingGetCameraWorldPosition); } },
            { "CAMERA_VIEW_POSITION",
              [](MaterialParameter* param, RenderState* rs)
              { param->bindValue(rs, &RenderState::autoBindingGetCameraViewPosition); } },
            { "MATRIX_PALETTE",
              [](MaterialParameter* param, RenderState* rs)
              {
                  param->bindValue(rs,
                                   &RenderState::autoBindingGetMatrixPalette,
                                   &RenderState::autoBindingGetMatrixPaletteSize);
              } },
            { "SCENE_AMBIENT_COLOR",
              [](MaterialParameter* param, RenderState* rs)
              { param->bindValue(rs, &RenderState::autoBindingGetAmbientColor); } }
        };
...
const Vector4* RenderState::autoBindingGetMatrixPalette() const
{
    ...
    if (model)
    {
        ...
        if (skin) return skin->getMatrixPalette();
    }
    return nullptr;
}
...
unsigned int RenderState::autoBindingGetMatrixPaletteSize() const
{
    ...
    {
        MeshSkin* skin = model->getSkin();
        if (skin) return skin->getMatrixPaletteSize();
    }
    return 0;
}
...
void RenderState::bind(Pass* pass)
{
    ...
    while (rs)
    {
        if (rs->_state) stateOverrideBits |= rs->_state->_bits;
        rs = rs->_parent;
    }
    ...
    while ((rs = getTopmost(rs)))
    {
        ...
        if (rs->_state) rs->_state->bindNoRestore();
    }
}
...
RenderState* RenderState::getTopmost(RenderState* below) noexcept
{
    ...
    while (rs)
    {
        ...
        if (rs->_parent == below || rs->_parent == nullptr) return rs;
        rs = rs->_parent;
    }
...
void RenderState::StateBlock::bindNoRestore()
{
    ...
    if ((_bits & RS_BLEND_FUNC)
        && (_blendSrc != _defaultState->_blendSrc || _blendDst != _defaultState->_blendDst))
    {
    ...
    if ((_bits & RS_STENCIL_FUNC)
        && (_stencilFunction != _defaultState->_stencilFunction
    ...
    if ((_bits & RS_STENCIL_OP)
        && (_stencilOpSfail != _defaultState->_stencilOpSfail
...
void RenderState::StateBlock::restore(long stateOverrideBits)
{
    ...
    {
        GL_ASSERT(glStencilOp((GLenum)RenderState::STENCIL_OP_KEEP,
                              (GLenum)RenderState::STENCIL_OP_KEEP,
                              (GLenum)RenderState::STENCIL_OP_KEEP));
...
static bool parseBoolean(const std::string& value)
{
    if (value.length() == 4)
    {
        return (tolower(value[0]) == 't' && tolower(value[1]) == 'r' && tolower(value[2]) == 'u'
                && tolower(value[3]) == 'e');
    }
...
static int parseInt(const std::string& value)
{
    ...
    if (scanned != 1)
    {
        GP_ERROR("Error attempting to parse int '%s'. (Will default to 0 if errors are treated as "
                 "warnings)",
                 value);
        return 0;
    }
...
static unsigned int parseUInt(const std::string& value)
{
    ...
    if (scanned != 1)
    {
        GP_ERROR("Error attempting to parse unsigned int '%s'. (Will default to 0 if errors are "
                 "treated as warnings)",
                 value);
        return 0;
    }
...
static RenderState::Blend parseBlend(const std::string& value)
{
    ...
    {
        GP_ERROR("Unsupported blend value (%s). (Will default to BLEND_ONE if errors are treated "
                 "as warnings)",
                 value);
        return RenderState::BLEND_ONE;
    }
...
static RenderState::DepthFunction parseDepthFunc(const std::string& value)
{
    ...
    {
        GP_ERROR("Unsupported depth function value (%s). Will default to DEPTH_LESS if errors are "
                 "treated as warnings)",
                 value);
        return RenderState::DEPTH_LESS;
    }
...
static RenderState::CullFaceSide parseCullFaceSide(const std::string& value)
{
    ...
    {
        GP_ERROR("Unsupported cull face side value (%s). Will default to BACK if errors are "
                 "treated as warnings.",
                 value);
        return RenderState::CULL_FACE_SIDE_BACK;
    }
...
static RenderState::FrontFace parseFrontFace(const std::string& value)
{
    ...
    {
        GP_ERROR("Unsupported front face side value (%s). Will default to CCW if errors are "
                 "treated as warnings.",
                 value);
        return RenderState::FRONT_FACE_CCW;
    }
...
static RenderState::StencilFunction parseStencilFunc(const std::string& value)
{
    ...
    {
        GP_ERROR("Unsupported stencil function value (%s). Will default to STENCIL_ALWAYS if "
...
static RenderState::StencilOperation parseStencilOp(const std::string& value)
{
    ...
    {
        GP_ERROR("Unsupported stencil operation value (%s). Will default to STENCIL_OP_KEEP if "
                 "errors are treated as warnings)",
                 value);
        return RenderState::STENCIL_OP_KEEP;
    }
...
RenderState::AutoBindingResolver::~AutoBindingResolver()
{
    std::vector<RenderState::AutoBindingResolver*>::iterator itr =
        std::find(_customAutoBindingResolvers.begin(), _customAutoBindingResolvers.end(), this);
    if (itr != _customAutoBindingResolvers.end()) _customAutoBindingResolvers.erase(itr);
}
