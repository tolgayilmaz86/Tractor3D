                                                       ...
                                                       Effect* effect)
{
    ...
    if (__maxVertexAttribs == 0)
    {
        ...
        if (__maxVertexAttribs <= 0)
        {
            GP_ERROR("The maximum number of vertex attributes supported by OpenGL on the current "
                     "device is 0 or less.");
            return nullptr;
        }
    }
    ...
    for (size_t i = 0, count = vertexFormat.getElementCount(); i < count; ++i)
    {
        ...
        tractor::VertexAttribute attrib;
        ...
        if (attrib == -1)
        {
            // GP_WARN("Warning: Vertex element with usage '%s' in mesh '%s' does not correspond to
            // an attribute in effect '%s'.", VertexFormat::toString(e.usage), mesh->getUrl(), effect->getId());
        }
        else
        {
            void* pointer =
                vertexPointer ? (void*)(((unsigned char*)vertexPointer) + offset) : (void*)offset;
            b->setVertexAttribPointer(attrib,
                                      (GLint)e.size,
                                      GL_FLOAT,
                                      GL_FALSE,
