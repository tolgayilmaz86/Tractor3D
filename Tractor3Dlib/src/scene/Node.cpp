...
constexpr auto NODE_DIRTY_BOUNDS = 2;
constexpr auto NODE_DIRTY_HIERARCHY = 4;
...
//-----------------------------------------------------------------------------
unsigned int Node::findNodes(const std::string& id,
...
//-----------------------------------------------------------------------------
void Node::removeTag(const std::string& name)
{
    // Removing tag
    if (_tags)
    {
...
PhysicsCollisionObject* Node::setCollisionObject(Properties* properties)
{
    ...
    {
        GP_ERROR("Failed to load collision object from properties object: must be non-null object "
    ...
    {
        GP_ERROR("Failed to load collision object from properties object; required attribute "
                 "'type' is missing.");
        return nullptr;
    }
