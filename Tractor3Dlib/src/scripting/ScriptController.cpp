...
#define GENERATE_LUA_GET_POINTER(type, checkFunc)                                                  \
    ...
    {                                                                                              \
        ...
        lua_pushfstring(sc->_lua,                                                                  \
                        "Expected a " #type                                                        \
                        " pointer (an array represented as a Lua table), got '%s' instead.",       \
    ...
    int size = luaL_checkinteger(sc->_lua, -1);                                                    \
    lua_pop(sc->_lua, 1);                                                                          \
    if (size <= 0) return LuaArray<type>((type*)nullptr);                                          \
                                                                                                   \
...
#define POP_NESTED_VARIABLE() lua_settop(_lua, top)
 ...
 * variable name in the table C.
 *
 * This function will not restore the stack if there is an error.
 *
 * @param lua  The Lua state.
 * @param name The name of a variable or a '.' separated list of nested tables ending with a
 * variable name. The name value may be in the format "A.B.C.D" where A is a table and B, C are
 * child tables. D is any type, which will be accessed by the calling function.
 * @param env Optional script environment ID, or zero for the global script environment.
 *
 * @return True if the tables were pushed on the stack or the variable was pushed. Returns false on
 * error.
 */
...
{
    PUSH_NESTED_VARIABLE(name, defaultValue, script);
    char c = lua_isnumber(_lua, -1) ? (char)luaL_checkinteger(_lua, -1) : defaultValue;
    POP_NESTED_VARIABLE();
    return c;
}
...
{
    PUSH_NESTED_VARIABLE(name, defaultValue, script);
    short n = lua_isnumber(_lua, -1) ? (short)luaL_checkinteger(_lua, -1) : defaultValue;
    POP_NESTED_VARIABLE();
    return n;
}
...
{
    PUSH_NESTED_VARIABLE(name, defaultValue, script);
    int n = lua_isnumber(_lua, -1) ? luaL_checkinteger(_lua, -1) : defaultValue;
    POP_NESTED_VARIABLE();
    return n;
}
...
{
    PUSH_NESTED_VARIABLE(name, defaultValue, script);
    long n = lua_isnumber(_lua, -1) ? luaL_checkinteger(_lua, -1) : defaultValue;
    POP_NESTED_VARIABLE();
    return n;
}
                                                ...
                                                Script* script)
{
    PUSH_NESTED_VARIABLE(name, defaultValue, script);
    unsigned char c =
        lua_isnumber(_lua, -1) ? (unsigned char)luaL_checkunsigned(_lua, -1) : defaultValue;
    POP_NESTED_VARIABLE();
    return c;
}
                                                  ...
                                                  Script* script)
{
    PUSH_NESTED_VARIABLE(name, defaultValue, script);
    unsigned short n =
        lua_isnumber(_lua, -1) ? (unsigned short)luaL_checkunsigned(_lua, -1) : defaultValue;
    POP_NESTED_VARIABLE();
    return n;
}
                                              ...
                                              Script* script)
{
    PUSH_NESTED_VARIABLE(name, defaultValue, script);
    unsigned int n =
        lua_isnumber(_lua, -1) ? (unsigned int)luaL_checkunsigned(_lua, -1) : defaultValue;
    POP_NESTED_VARIABLE();
    return n;
}
                                                ...
                                                Script* script)
{
    PUSH_NESTED_VARIABLE(name, defaultValue, script);
    unsigned long n =
        lua_isnumber(_lua, -1) ? (unsigned long)luaL_checkunsigned(_lua, -1) : defaultValue;
    POP_NESTED_VARIABLE();
    return n;
}
...
{
    PUSH_NESTED_VARIABLE(name, nullptr, script);
    void* ptr = lua_islightuserdata(_lua, -1) ? lua_touserdata(_lua, -1)
                                              : ScriptUtil::getUserDataObjectPointer(-1, type);
    POP_NESTED_VARIABLE();
    return ptr;
}
...
void ScriptController::setObjectPointer(const char* type, const char* name, void* v, Script* script)
{
    ...
    ScriptUtil::LuaObject* object =
        (ScriptUtil::LuaObject*)lua_newuserdata(_lua, sizeof(ScriptUtil::LuaObject));
    object->instance = v;
    object->owns = false;
...
{
    _lua = luaL_newstate();
    if (!_lua) GP_ERROR("Failed to initialize Lua scripting engine.");
    luaL_openlibs(_lua);
    ...
    if (luaL_dostring(_lua, lua_loadfile_function))
        GP_ERROR("Failed to load custom loadfile() function with error: '%s'.",
                 lua_tostring(_lua, -1));
    ...
    if (argsStr.length() > 0)
    {
        if (luaL_dostring(_lua, argsStr.c_str()))
            GP_ERROR("Failed to pass command-line arguments with error: '%s'.",
                     lua_tostring(_lua, -1));
    }
}
...
bool ScriptController::executeFunctionHelper(int resultCount,
                                             ...
                                             Script* script)
{
    if (!_lua) return false; // handles calling this method after script is finalized
        ...
        while (true)
        {
            ...
            switch (*sig++)
            {
                ...
                case '<':
                {
                    ...
                    {
                        ScriptUtil::LuaObject* object =
                            (ScriptUtil::LuaObject*)lua_newuserdata(_lua,
                                                                    sizeof(ScriptUtil::LuaObject));
                        object->instance = ptr;
                        object->owns = false;
...
#define SCRIPT_EXECUTE_FUNCTION_NO_PARAM(script, type, checkfunc)                                  \
    ...
    if (out && success) *out = (type)checkfunc(_lua, -1);                                          \
    lua_settop(_lua, top);                                                                         \
    return success;
                               ...
                               const std::vector<std::string>& scopePath)
{
    ...
    if (members) luaL_setfuncs(sc->_lua, members, 0);
    ...
    if (statics) luaL_setfuncs(sc->_lua, statics, 0);
...
void* ScriptUtil::getUserDataObjectPointer(int index, const char* type)
{
    ...
    for (size_t i = 0, count = types.size(); i < count; i++)
    {
        ...
        if (lua_rawequal(sc->_lua, -1, -2))
        {
            ...
            return luaConvertObjectPointer(((ScriptUtil::LuaObject*)p)->instance,
                                           relatedType.c_str(),
                                           type);
        }
...
const char* ScriptUtil::getString(int index, bool isStdString)
{
    ...
    else if (lua_type(Game::getInstance()->getScriptController()->_lua, index) == LUA_TNIL
             && !isStdString)
        return nullptr;
    else
    {
...
bool ScriptUtil::luaCheckBool(lua_State* state, int n)
{
    ...
    {
        const char* msg = lua_pushfstring(state,
                                          "%s expected, got %s",
                                          lua_typename(state, LUA_TBOOLEAN),
                                          luaL_typename(state, n));
                                                     ...
                                                     bool* success)
{
    *success = false;
    ...
    if (lua_type(sc->_lua, index) == LUA_TNIL)
    {
        if (nonNull)
        {
            GP_WARN("Attempting to pass nullptr for required non-nullptr parameter at index %d "
                    "(likely a reference or by-value parameter).",
                    index);
        }
        else
        {
    ...
    {
        GP_WARN("Attempting to pass nullptr for required non-nullptr parameter at index %d (likely "
                "a reference or by-value parameter).",
                index);
        return LuaArray<T>((T*)nullptr);
    }
    else
...
bool ScriptController::executeFunction(Script* script, const char* func, T* out)
{
    ...
    int top = lua_gettop(_lua);
    bool success = executeFunctionHelper(1, func, nullptr, nullptr, script);
    if (out && success) *out = (T)((ScriptUtil::LuaObject*)lua_touserdata(_lua, -1))->instance;
    lua_settop(_lua, top);
    return success;
}
