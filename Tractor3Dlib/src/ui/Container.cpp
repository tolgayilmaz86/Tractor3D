...
void Container::addControls(Properties* properties)
{
    ...
    {
        auto controlName = controlSpace->getNamespace();
...
unsigned int Container::addControl(Control* control)
{
    ...
    if (control->getFocusIndex() == -1)
    {
        ...
        // std::views::transform: Creates a range that applies a function([](const control* c) {
        // return c->getFocusIndex(); }) to each element of _controls, resulting in a range of
        // _focusIndex values. std::ranges::max : This finds the maximum value directly from the
        // transformed range of focus indices, simplifying the code.
        int maxFocusIndex = 0;
        if (!_controls.empty())
...
bool Container::isScrolling() const
{
    ...
    if (_parent && _parent->isScrolling()) return true;
...
bool Container::setFocus()
{
    ...
    if (Form::getFocusControl()
        && (Form::getFocusControl() == this || Form::getFocusControl()->isChild(this)))
        return true;

    // First try to set focus to our active control
    if (_activeControl)
    {
        if (_activeControl->setFocus()) return true;
    }

    // Try to set focus to one of our children
    for (size_t i = 0, count = _controls.size(); i < count; ++i)
    {
        if (_controls[i]->setFocus()) return true;
    }
...
void Container::setActiveControl(Control* control)
{
    if (std::find(_controls.begin(), _controls.end(), control) != _controls.end())
    {
        ...
        if (Form::getFocusControl() && Form::getFocusControl() != control
            && Form::getFocusControl()->isChild(this))
        ...
        if (_autoSize & AUTO_SIZE_WIDTH)
        {
            ...
            for (size_t i = 0, count = _controls.size(); i < count; ++i)
            {
                ...
                if (ctrl->isVisible() && !ctrl->isWidthPercentage())
                {
                    ...
                    if (!ctrl->isXPercentage()) w += ctrl->getX();
                    if (width < w) width = w;
                }
            }
            width += getBorder(NORMAL).left + getBorder(NORMAL).right + getPadding().left
                     + getPadding().right;
        ...
        if (_autoSize & AUTO_SIZE_HEIGHT)
        {
            ...
            for (size_t i = 0, count = _controls.size(); i < count; ++i)
            {
                ...
                if (ctrl->isVisible() && !ctrl->isHeightPercentage())
                {
                    ...
                    if (!ctrl->isYPercentage()) h += ctrl->getY();
                    if (height < h) height = h;
                }
            }
            height += getBorder(NORMAL).top + getBorder(NORMAL).bottom + getPadding().top
                      + getPadding().bottom;
...
unsigned int Container::draw(Form* form, const Rectangle& clip)
{
    ...
    if (_scroll != SCROLL_NONE && (_scrollBarOpacity > 0.0f))
    {
        ...
        if (_scrollBarBounds.height > 0 && ((_scroll & SCROLL_VERTICAL) == SCROLL_VERTICAL))
        {
            ...
            Rectangle bounds(_viewportBounds.right()
                                 + (_absoluteBounds.right() - _viewportBounds.right()) * 0.5f
        ...
        if (_scrollBarBounds.width > 0 && ((_scroll & SCROLL_HORIZONTAL) == SCROLL_HORIZONTAL))
        {
            ...
            Rectangle bounds(_viewportBounds.x + _scrollBarBounds.x,
                             _viewportBounds.bottom()
                                 + (_absoluteBounds.bottom() - _viewportBounds.bottom()) * 0.5f
                                 - leftRegion.height * 0.5f,
                             leftRegion.width,
                             leftRegion.height);
            batch->draw(bounds.x,
                        bounds.y,
...
static bool canReceiveFocus(Control* control)
{
    ...
    if (control->canFocus()) return true;
...
bool Container::moveFocusNextPrevious(Direction direction)
{
    ...
    if (current)
    {
        ...
        int focusableControlCount =
            0; // track the number of valid focusable controls in this container
        ...
        for (size_t i = 0, count = _controls.size(); i < count; ++i)
        {
            ...
            if ((direction == NEXT && ctrl->_focusIndex > current->_focusIndex
                 && ctrl->_focusIndex < nextIndex)
                || (direction == PREVIOUS && ctrl->_focusIndex < current->_focusIndex
                    && ctrl->_focusIndex > nextIndex))
            {
                nextCtrl = ctrl;
        ...
        if (nextCtrl)
        {
            if (nextCtrl->isContainer() && static_cast<Container*>(nextCtrl)->moveFocus(direction))
                return true;
            if (nextCtrl->setFocus()) return true;
        }

        // Search up into our parent container for a focus move
        if (_parent && _parent->moveFocus(direction)) return true;

        // We didn't find a control to move to, so we must be the first or last focusable control in
        // our parent. Wrap focus to the other side of the container.
...
bool Container::moveFocusDirectional(Direction direction)
{
    ...
    if (startControl == nullptr) return false;
    ...
    if (next)
    {
        ...
        if (next->isContainer())
        {
            if (static_cast<Container*>(next)->moveFocusDirectional(direction)) return true;
        }

        if (next->setFocus()) return true;
    }
    else
    {
        // If no control was found, try searching in our parent container
        if (_parent && _parent->moveFocusDirectional(direction)) return true;
    }
...
void Container::updateScroll()
{
    ...
    float clipWidth = _absoluteBounds.width - containerBorder.left - containerBorder.right
                      - containerPadding.left - containerPadding.right - vWidth;
    float clipHeight = _absoluteBounds.height - containerBorder.top - containerBorder.bottom
                       - containerPadding.top - containerPadding.bottom - hHeight;
    ...
    if (!_scrollingVelocity.isZero())
    {
        ...
        if (!_scrolling)
        {
            ...
            if (fabs(_scrollingVelocity.x) < 100.0f) _scrollingVelocity.x = 0.0f;
            if (fabs(_scrollingVelocity.y) < 100.0f) _scrollingVelocity.y = 0.0f;
        }
    ...
    if (_scrollPosition != lastScrollPosition) dirty = true;

    float scrollWidth = 0;
    if (clipWidth < _totalWidth) scrollWidth = (clipWidth / _totalWidth) * clipWidth;

    float scrollHeight = 0;
    if (clipHeight < _totalHeight) scrollHeight = (clipHeight / _totalHeight) * clipHeight;
    ...
    switch (evt)
    {
        case Touch::TOUCH_PRESS:
            if (_contactIndex == INVALID_CONTACT_INDEX)
            {
                ...
                _scrollBarOpacity = 1.0f;
                if (dirty) setDirty(DIRTY_BOUNDS);
                return false;
            }
            break;
        ...
        case Touch::TOUCH_MOVE:
            if (_scrolling && _contactIndex == (int)contactIndex)
            {
                ...
                if (!_scrollingStartTimeX) _scrollingStartTimeX = gameTime;

                if (!_scrollingStartTimeY) _scrollingStartTimeY = gameTime;
        ...
        case Touch::TOUCH_RELEASE:
            if (_contactIndex == (int)contactIndex)
            {
                ...
                float vx = dx;
                float vy = dy;
                if (elapsedSecsX > 0) vx = (float)dx / elapsedSecsX;
                if (elapsedSecsY > 0) vy = (float)dy / elapsedSecsY;
    ...
    switch (evt)
    {
        case Mouse::MOUSE_PRESS_LEFT_BUTTON:
        {
            ...
            if (_scrollBarVertical)
            {
                ...
                Rectangle vBounds(localVpRight + rightPadding * 0.5f - vWidth * 0.5f,
            ...
            if (_scrollBarHorizontal)
            {
                ...
                Rectangle hBounds(leftPadding + _scrollBarBounds.x,
...
Container::Scroll Container::getScroll(const std::string& scroll)
{
    ...
    else if (scroll == "SCROLL_HORIZONTAL")
    {
        return Container::SCROLL_HORIZONTAL;
    }
    else if (scroll == "SCROLL_VERTICAL")
    {
        return Container::SCROLL_VERTICAL;
    }
    else if (scroll == "SCROLL_BOTH")
    {
