...
namespace tractor
{
	...
	Control::AutoSize Control::parseAutoSize(const std::string& str)
	{
		...
		if (upperStr == "AUTO_SIZE_WIDTH")
			return AUTO_SIZE_WIDTH;
		if (upperStr == "AUTO_SIZE_HEIGHT")
			return AUTO_SIZE_HEIGHT;
		if (upperStr == "AUTO_SIZE_BOTH")
	...
	void Control::initialize(const std::string& typeName, Theme::Style* style, Properties* properties)
	{
		...
		if (properties)
		{
			...
			if (!position.empty() && parseCoordPair(position, &bounds[0], &bounds[1], &boundsBits[0], &boundsBits[1]))
			{
			...
			if (!size.empty() && parseCoordPair(size, &bounds[0], &bounds[1], &boundsBits[0], &boundsBits[1]))
			{
	...
	//----------------------------------------------------------------
	bool Control::parseCoordPair(const std::string& s, float* v1, float* v2, bool* v1Percentage, bool* v2Percentage)
	{
