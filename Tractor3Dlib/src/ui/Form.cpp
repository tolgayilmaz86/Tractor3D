...
static Control* __focusControl = nullptr;
...
void Form::startBatch(SpriteBatch* batch)
{
    ...
    // sorted correctly, such as all text on top)
    if (!batch->isStarted())
    {
        batch->setProjectionMatrix(_projectionMatrix);
        batch->start();

        if (_batched) _batches.push_back(batch);
    }
}
...
Control* Form::findInputControl(int* x, int* y, bool focus, unsigned int contactIndex)
{
    for (int i = (int)__forms.size() - 1; i >= 0; --i)
    {
        ...
        int formX = *x;
        int formY = *y;
        if (!screenToForm(form, &formX, &formY)) continue;
...
Control* Form::findInputControl(Control* control, int x, int y, bool focus, unsigned int contactIndex)
{
    ...
    {
        if (control->_absoluteClipBounds.contains(x, y)) result = control;
    }
    ...
    if (control->isContainer())
    {
        ...
        //   std::ranges::views::iota(0u, container->getControlCount()),
        //   [&](unsigned int i) {
        //     return findInputControl(container->getControl(i), x, y, focus, contactIndex);
        //   });

        auto it = std::ranges::find_if(std::ranges::views::iota(0u, container->getControlCount()),
                                       [&](unsigned int i)
                                       {
                                           Control* ctrl = findInputControl(container->getControl(i),
                                                                            x,
                                                                            y,
                                                                            focus,
                                                                            contactIndex);
                                           if (ctrl)
                                           {
                                               result = ctrl; // Capture the found control.
                                               return true;   // Signal that we've found a match.
                                           }
                                           return false;
                                       });
    }
...
Control* Form::handlePointerPressRelease(int* x, int* y, bool pressed, unsigned int contactIndex)
{
    ...
    if (pressed)
    {
        ...
        if ((ctrl = findInputControl(&newX, &newY, false, contactIndex)) != nullptr)
        {
    ...
    {
        Control* active =
            (__activeControl[contactIndex] && __activeControl[contactIndex]->_state == ACTIVE)
                ? __activeControl[contactIndex]
                : nullptr;
...
bool Form::pointerEventInternal(bool mouse, int evt, int x, int y, int param)
{
    ...
    if (mouse && Game::getInstance()->isMouseCaptured()) return false;

    // Is this a press event (TOUCH_PRESS has the same value as MOUSE_PRESS_LEFT_BUTTON)
    bool pressEvent =
        evt == Touch::TOUCH_PRESS
        || (mouse
            && (evt == Mouse::MOUSE_PRESS_MIDDLE_BUTTON || evt == Mouse::MOUSE_PRESS_RIGHT_BUTTON));
    ...
    if (ctrl)
    {
        ...
        while (ctrl)
        {
            ...
            if (mouse)
            {
                if (ctrl->mouseEvent((Mouse::MouseEvent)evt, localX, localY, param)) return true;

                // Forward to touch event hanlder if unhandled by mouse handler
                switch (evt)
                {
                    case Mouse::MOUSE_PRESS_LEFT_BUTTON:
                        if (ctrl->touchEvent(Touch::TOUCH_PRESS, localX, localY, 0)) return true;
                        break;
                    case Mouse::MOUSE_RELEASE_LEFT_BUTTON:
                        if (ctrl->touchEvent(Touch::TOUCH_RELEASE, localX, localY, 0)) return true;
                        break;
                    case Mouse::MOUSE_MOVE:
                        if (ctrl->touchEvent(Touch::TOUCH_MOVE, localX, localY, 0)) return true;
                        break;
                }
            }
            else
            ...
            while (tmp)
            {
                if (tmp->isContainer())
                {
                    ...
                    if (container->_scroll != SCROLL_NONE)
                    {
                        if (mouse)
                        {
                            if (container->mouseEventScroll((Mouse::MouseEvent)evt,
                                                            formX - tmp->_absoluteBounds.x,
                                                            formY - tmp->_absoluteBounds.y,
                                                            param))
                                return true;
                        }
                        else
                        {
                            if (container->touchEventScroll((Touch::TouchEvent)evt,
                                                            formX - tmp->_absoluteBounds.x,
                                                            formY - tmp->_absoluteBounds.y,
                                                            param))
                                return true;
            ...
            if (ctrl->getConsumeInputEvents()) return true;
...
bool Form::keyEventInternal(Keyboard::KeyEvent evt, int key)
{
            ...
            case Keyboard::KEY_CHAR:
                switch (key)
                {
                    case Keyboard::KEY_TAB:
                        ...
                        {
                            if (__focusControl->_parent->moveFocus(
                                    __shiftKeyDown ? Container::PREVIOUS : Container::NEXT))
                                return true;
                        }
                        break;
                }
    ...
    while (ctrl)
    {
        ...
        {
            if (ctrl->keyEvent(evt, key)) return true;
        }
...
bool Form::pollGamepad(Gamepad* gamepad)
{
    ...
    if (__focusControl->_parent) parentContainer = __focusControl->_parent;

    // Get scroll container
    Container* scrollContainer = nullptr;
    if (__focusControl->isContainer())
    {
        scrollContainer = static_cast<Container*>(__focusControl);
        if (scrollContainer->_scroll == SCROLL_NONE) scrollContainer = nullptr;
    }
    ...
    bool selectButtonDown =
        gamepad->isButtonDown(Gamepad::BUTTON_A) || gamepad->isButtonDown(Gamepad::BUTTON_X);
    ...
    if (!selectButtonDown)
    {
        ...
        if (parentContainer)
        {
            ...
            if (gamepad->isButtonDown(Gamepad::BUTTON_UP)
                || (joystickCount > 1 && joystick.y > JOYSTICK_THRESHOLD))
            {
                focusPressed = true;
                if (focusChangeElapsedTime > GAMEPAD_FOCUS_REPEAT_DELAY
                    && parentContainer->moveFocus(UP))
                {
                    lastFocusChangeTime = currentTime;
                }
            }

            if (gamepad->isButtonDown(Gamepad::BUTTON_DOWN)
                || (joystickCount > 1 && joystick.y < -JOYSTICK_THRESHOLD))
            {
                focusPressed = true;
                if (focusChangeElapsedTime > GAMEPAD_FOCUS_REPEAT_DELAY
                    && parentContainer->moveFocus(DOWN))
                {
                    lastFocusChangeTime = currentTime;
                }
            }
            ...
            if (gamepad->isButtonDown(Gamepad::BUTTON_LEFT)
                || (joystickCount > 1 && joystick.x < -JOYSTICK_THRESHOLD))
            {
                focusPressed = true;
                if (focusChangeElapsedTime > GAMEPAD_FOCUS_REPEAT_DELAY
                    && parentContainer->moveFocus(LEFT))
                {
                    lastFocusChangeTime = currentTime;
                }
            }

            if (gamepad->isButtonDown(Gamepad::BUTTON_RIGHT)
                || (joystickCount > 1 && joystick.x > JOYSTICK_THRESHOLD))
            {
                focusPressed = true;
                if (focusChangeElapsedTime > GAMEPAD_FOCUS_REPEAT_DELAY
                    && parentContainer->moveFocus(RIGHT))
                {
                    lastFocusChangeTime = currentTime;
                }
            }
        ...
        {
            if (joystickCount > 1) gamepad->getJoystickValues(1, &joystick);
            if (std::fabs(joystick.x) > JOYSTICK_THRESHOLD
                || std::fabs(joystick.y) > JOYSTICK_THRESHOLD)
            {
                scrollContainer->startScrolling(GAMEPAD_SCROLL_SPEED * joystick.x,
                                                GAMEPAD_SCROLL_SPEED * joystick.y,
                                                !scrolling);
    ...
    {
        scrolling = false;
        if (scrollContainer) scrollContainer->stopScrolling();
    }
...
bool Form::gamepadButtonEventInternal(Gamepad* gamepad)
{
    ...
    while (ctrl)
    {
        if (ctrl->isEnabled() && ctrl->isVisible())
        {
            if (ctrl->gamepadButtonEvent(gamepad)) return true;
        }
...
bool Form::gamepadTriggerEventInternal(Gamepad* gamepad, unsigned int index)
{
    ...
    while (ctrl)
    {
        if (ctrl->isEnabled() && ctrl->isVisible())
        {
            if (ctrl->gamepadTriggerEvent(gamepad, index)) return true;
        }
...
bool Form::gamepadJoystickEventInternal(Gamepad* gamepad, unsigned int index)
{
    ...
    while (ctrl)
    {
        if (ctrl->isEnabled() && ctrl->isVisible())
        {
            if (ctrl->gamepadJoystickEvent(gamepad, index)) return true;
        }
...
void Form::setFocusControl(Control* control)
{
    ...
    if (__focusControl)
    {
        ...
        if (parent && parent->_scroll != SCROLL_NONE && !parent->_viewportBounds.isEmpty())
        {
            ...
            else if (bounds.x + bounds.width
                     > parent->_scrollPosition.x + parent->_viewportBounds.width)
            {
                // Control is off to the right.
                parent->_scrollPosition.x =
                    -(bounds.x + bounds.width - parent->_viewportBounds.width);
            }
            ...
            if (bounds.y < parent->_viewportBounds.y - parent->_scrollPosition.y)
            {
                ...
                parent->_scrollPosition.y = -bounds.y;
                     ...
                     > parent->_viewportBounds.height - parent->_scrollPosition.y)
            {
                // Control is below the viewport.
                parent->_scrollPosition.y =
                    -(bounds.y + bounds.height - parent->_viewportBounds.height);
            }
        }
    }
}
