...
Slider::Slider()
    : _min(0.0f), _max(0.0f), _step(0.0f), _value(0.0f), _delta(0.0f), _minImage(nullptr),
...
void Slider::updateValue(int x, int y)
{
    ...
    float markerPosition =
        (x - markerRegion.width * 0.5f) / (_viewportBounds.width - markerRegion.width);
...
void Slider::updateBounds()
{
    ...
    if (_autoSize & AUTO_SIZE_HEIGHT)
    {
        ...
        if (_valueTextVisible) height += getFontSize(NORMAL);
...
unsigned int Slider::drawImages(Form* form, const Rectangle& clip)
{
    ...
    batch->draw(pos.x,
                pos.y,
                _viewportBounds.width - minCapRegion.width - maxCapRegion.width,
                trackRegion.height,
                track.u1,
                track.v1,
                track.u2,
