...
namespace tractor
{
	...
	Sprite* Sprite::create(const std::string& imagePath, float width, float height,
	...
	static bool parseBlendMode(const std::string& str, Sprite::BlendMode* blend)
	{
		...
		{
			GP_ERROR("Failed to get corresponding sprite blend mode for unsupported value '%s'.", str.c_str());
			*blend = Sprite::BLEND_NONE;
			return false;
		}
		else
	...
	static bool parseFlipFlags(const std::string& str, Sprite::FlipFlags* flip)
	{
		...
		{
			GP_ERROR("Failed to get corresponding sprite flip flag for unsupported value '%s'.", str.c_str());
			*flip = Sprite::FLIP_NONE;
			return false;
		}
		else
	...
	static bool parseOffset(const std::string& str, Sprite::Offset* offset)
	{
		...
		{
			GP_ERROR("Failed to get corresponding sprite offset for unsupported value '%s'.", str.c_str());
			*offset = Sprite::OFFSET_LEFT;
			return false;
		}
