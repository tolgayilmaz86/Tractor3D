            ...
            switch (key)
            {
                ...
                case Keyboard::KEY_DELETE:
                {
                    if (_caretLocation < _text.length())
                    {
                        ...
                        {
                            newCaretLocation =
                                findNextWord(getDisplayedText(), _caretLocation, false);
                        }
                        else
                        {
...
unsigned int TextBox::drawImages(Form* form, const Rectangle& clip)
{
    ...
    if (_caretImage && (state == ACTIVE || hasFocus()))
    {
        ...
        if (!region.isEmpty())
        {
            ...
            Vector2 point;
            font->getLocationAtIndex(getDisplayedText(),
                                     _textBounds,
                                     fontSize,
                                     &point,
                                     _caretLocation,
            ...
            batch->draw(point.x - caretWidth * 0.5f,
                        point.y,
                        caretWidth,
                        fontSize,
                        uvs.u1,
...
void TextBox::setCaretLocation(int x, int y)
{
    ...
    int index = font->getIndexAtLocation(displayedText,
                                         _textBounds,
                                         fontSize,
                                         point,
                                         &point,
    ...
    if (index == -1)
    {
        ...
        if (point.x > textBounds.x + textBounds.width && point.y > textBounds.y + textBounds.height)
        {
            font->getLocationAtIndex(displayedText,
                                     _textBounds,
                                     fontSize,
                                     &point,
                                     (unsigned int)_text.length(),
                                     textAlignment,
                                     true,
                                     rightToLeft);
            return;
        ...
        index = font->getIndexAtLocation(displayedText,
                                         _textBounds,
                                         fontSize,
                                         point,
                                         &point,
...
void TextBox::getCaretLocation(Vector2* p)
{
    ...
    getFont(state)->getLocationAtIndex(getDisplayedText(),
                                       _textBounds,
                                       getFontSize(state),
                                       p,
                                       _caretLocation,
...
TextBox::InputMode TextBox::getInputMode(const std::string& inputMode)
{
    ...
    {
        GP_ERROR("Failed to get corresponding textbox inputmode for unsupported value '%s'.",
                 inputMode);
    }
