...
void* operator new(std::size_t size) throw(std::bad_alloc) { return operator new(size, "", 0); }
...
void operator delete(void* p) throw() { debugFree(p); }

void operator delete[](void* p) throw() { operator delete(p); }

void operator delete(void* p, const char* file, int line) throw() { operator delete(p); }
    ...
    if (rec->trackStackTrace)
    {
        ...
        if (!initialized)
        {
            if (!SymInitialize(GetCurrentProcess(), nullptr, true))
                tractor::print("Stack trace tracking will not work.\n");
            initialized = true;
        }
...
#error "Machine architecture not supported!"
    ...
    if (__memoryAllocations) __memoryAllocations->prev = rec;
...
void debugFree(void* p)
{
    ...
    // Link this item out
    std::lock_guard<std::mutex> lock(getMemoryAllocationMutex());
    if (__memoryAllocations == rec) __memoryAllocations = rec->next;
...
void printStackTrace(MemoryAllocationRecord* rec)
{
    ...
    for (int i = 0; i < MAX_STACK_FRAMES; i++)
    {
        ...
        {
            tractor::print("[memory] STACK TRACE: <unknown location>\n");
        }
        else
        {
            ...
            else
            {
                ...
                if (pc != 0)
                {
                    ...
                    {
                        tractor::print("[memory] STACK TRACE: %s - <unknown file>:<unknown line "
                                       "number>\n",
                                       symbol->Name);
                    }
                    else
                    {
                        ...
                        tractor::print("[memory] STACK TRACE: %s - %s:%d\n",
                                       symbol->Name,
                                       file,
                                       line.LineNumber);
                    }
...
extern void printMemoryLeaks()
{
    ...
    {
        tractor::print(
            "[memory] All HEAP allocations successfully cleaned up (no leaks detected).\n");
    }
    else
    {
        tractor::print("[memory] WARNING: %d HEAP allocations still active in memory.\n",
                       __memoryAllocationCount);
...
#if defined(WIN32)
void setTrackStackTrace(bool trackStackTrace) { __trackStackTrace = trackStackTrace; }

void toggleTrackStackTrace() { __trackStackTrace = !__trackStackTrace; }
#endif
