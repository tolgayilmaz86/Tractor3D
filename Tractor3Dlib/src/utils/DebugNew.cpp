...
void debugFree(void* p)
{
	...
	{
		tractor::print("[memory] CORRUPTION: Attempting to free memory address with invalid memory allocation record.\n");
		return;
	}
		...
		else
		{
			...
			else
			{
				...
				if (pc != 0)
				{
					...
					{
						tractor::print("[memory] STACK TRACE: %s - <unknown file>:<unknown line number>\n", symbol->Name);
					}
					else
					{
...
extern void printMemoryLeaks()
{
	...
	{
		tractor::print("[memory] All HEAP allocations successfully cleaned up (no leaks detected).\n");
	}
	else
	{
		...
		while (rec)
		{
			...
			if (rec->trackStackTrace)
			{
				...
				tractor::print("[memory] LEAK: HEAP allocation leak at address %#x of size %d from line %d in file '%s'.\n", rec->address, rec->size, rec->line, rec->file);
#else
			tractor::print("[memory] LEAK: HEAP allocation leak at address %#x of size %d from line %d in file '%s'.\n", rec->address, rec->size, rec->line, rec->file);
#endif
			rec = rec->next;
