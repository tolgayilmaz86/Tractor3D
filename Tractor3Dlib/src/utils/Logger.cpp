...
void Logger::log(Level level, const char* message, ...)
{
    ...
    else if (state.logFunctionLua)
    {
        ...
        Game::getInstance()->getScriptController()->executeFunction<void>(state.logFunctionLua,
                                                                          "[Logger::Level]s",
                                                                          nullptr,
                                                                          level,
                                                                          str);
...
bool Logger::isEnabled(Level level) { return _state[level].enabled; }

void Logger::setEnabled(Level level, bool enabled) { _state[level].enabled = enabled; }
