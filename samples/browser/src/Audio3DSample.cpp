...
Audio3DSample::Audio3DSample()
    : _font(nullptr), _scene(nullptr), _cubeNode(nullptr), _gamepad(nullptr), _moveFlags(0),
...
void Audio3DSample::initialize()
{
    ...
    boxMaterial->getParameter("u_directionalLightDirection[0]")
...
void Audio3DSample::addSound(const std::string& file)
{
    ...
    {
        auto audioSource = AudioSource::create(path);
        assert(audioSource);
