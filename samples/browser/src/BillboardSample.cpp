...
BillboardSample::BillboardSample()
    : _font(nullptr), _scene(nullptr), _ground(nullptr), _gamepad(nullptr), _moveFlags(0),
      _prevX(0), _prevY(0), _buttonPressed(false)
{
}
...
void BillboardSample::render(float elapsedTime)
{
    ...
    for (size_t i = 0; i < BILLBOARD_COUNT; i++)
    {
        ...
        Matrix m = Matrix::createBillboard(node->getTranslationWorld(),
                                           ...
                                           camera->getNode()->getUpVectorWorld());
        Quaternion q;
        m.getRotation(&q);
        node->setRotation(q);

        if (node->getBoundingSphere().intersects(camera->getFrustum())) node->getDrawable()->draw();
    }
...
void BillboardSample::loadGround()
{
    ...
    Texture::Sampler* sampler =
        material->getParameter("u_diffuseTexture")->setValue("res/png/dirt.png", true);
    sampler->setFilterMode(Texture::LINEAR_MIPMAP_LINEAR, Texture::LINEAR);
    material->getParameter("u_textureRepeat")
        ->setValue(Vector2(GROUND_REPEAT_TEXTURE, GROUND_REPEAT_TEXTURE));
...
{
    auto mesh = Mesh::createQuad(-(BILLBOARD_WIDTH / 2.0f),
                                 -(BILLBOARD_HEIGHT / 2.0f),
                                 BILLBOARD_WIDTH,
                                 BILLBOARD_HEIGHT);
