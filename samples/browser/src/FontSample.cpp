...
void FontSample::render(float elapsedTime)
{
	...
	if (_simple)
	{
		...
		_font->drawText(_sampleString, _viewport.x, _viewport.y, Vector4::fromColor(0xff0000ff), _size, _rightToLeft);
	...
	else
	{
		...
		_font->drawText(_sampleString, _useViewport ? _viewport : area, Vector4::fromColor(0xffffffff), _size, _alignment, _wrap, _rightToLeft);
