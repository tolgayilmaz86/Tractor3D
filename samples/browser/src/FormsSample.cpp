...
{
    const std::string formFiles[] = {
        "res/common/forms/formBasicControls.form", "res/common/forms/formScrolling.form",
        "res/common/forms/formFlowLayout.form",    "res/common/forms/formVerticalLayout.form",
...
void FormsSample::update(float elapsedTime)
{
    ...
    if (_gamepad->isButtonDown(Gamepad::BUTTON_MENU1))
    {
        if (!(_keyFlags & KEY_BACK))
        {
            ...
            {
                if (Form::getFocusControl() == _activeForm
                    || Form::getFocusControl()->isChild(_activeForm))
                    _formSelect->setFocus();
                else
                    Form::clearFocus();
            }
            ...
            {
                if (!(_activeForm && _activeForm->setFocus())) _formSelect->setFocus();
            }
        }
    }
    else
    ...
    if (Form::getFocusControl() == nullptr)
    {
        if (!_joysticks[0].isZero())
        {
            _formNodeParent->translate(0.5f * speedFactor * _joysticks[0].x,
                                       0.5f * speedFactor * _joysticks[0].y,
                                       0);
        }
...
void FormsSample::controlEvent(Control* control, EventType evt)
{
    if (evt == CLICK)
    {
        if (control->getId() == "form0")
        {
            _formIndex = 0;
            formChanged();
        }
        else if (control->getId() == "form1")
        {
            _formIndex = 1;
            formChanged();
        }
        else if (control->getId() == "form2")
        {
            _formIndex = 2;
            formChanged();
        }
        else if (control->getId() == "form3")
        {
            _formIndex = 3;
            formChanged();
        }
        else if (control->getId() == "form4")
        {
            _formIndex = 4;
            formChanged();
        }
        else if (control->getId() == "form5")
        {
            _formIndex = 5;
            formChanged();
        }
        else if (control->getId() == "opacityButton")
        {
            float from[] = { 1.0f };
            float to[] = { 0.5f };
            control
                ->createAnimationFromTo("opacityButton",
                                        Form::ANIMATE_OPACITY,
                                        from,
                                        to,
                                        Curve::LINEAR,
                                        1000)
