...
void LightSample::touchEvent(Touch::TouchEvent evt, int x, int y, unsigned int contactIndex)
{
    switch (evt)
    {
        case Touch::TOUCH_PRESS:
        {
            ...
            if (_directional->isSelected()
                && pickRay.intersects(_directionalLightQuadModel->getMesh()->getBoundingBox()))
            {
                _usedForMoving = _directionalLightNode;
            }
            else if (_spot->isSelected()
                     && pickRay.intersects(_spotLightQuadModel->getMesh()->getBoundingBox()))
            {
                _usedForMoving = _spotLightNode;
            }
            else if (_point->isSelected()
                     && pickRay.intersects(_pointLightQuadModel->getMesh()->getBoundingBox()))
            {
        ...
        case Touch::TOUCH_MOVE:
        {
            ...
            // Roll in world frame
            Vector3 pitch = m.getRightVector();
...
void LightSample::controlEvent(Control* control, EventType evt)
{
    ...
    switch (evt)
    {
        case Control::Listener::VALUE_CHANGED:
            ...
            {
                Vector3 color(_redSlider->getValue(),
                              _greenSlider->getValue(),
                              _blueSlider->getValue());
                setColorValue(color);
            }
...
{
    _lighting->getTechnique(technique)
        ->getParameter("u_ambientColor")
        ->setValue(Vector3(0.0f, 0.0f, 0.0f));
    _lighting->getTechnique(technique)
        ->getParameter("u_directionalLightColor[0]")
        ->setValue(Vector3(_redSlider->getValue(), _greenSlider->getValue(), _blueSlider->getValue()));
    _lighting->getTechnique(technique)
        ->getParameter("u_directionalLightDirection[0]")
        ->bindValue(_directionalLightNode, &Node::getForwardVectorView);
}
...
void LightSample::initializeSpotTechnique(const std::string& technique)
{
    _lighting->getTechnique(technique)
        ...
        ->setValue(Vector3(0.0f, 0.0f, 0.0f));
    _lighting->getTechnique(technique)
        ->getParameter("u_spotLightColor[0]")
        ->setValue(Vector3(_redSlider->getValue(), _greenSlider->getValue(), _blueSlider->getValue()));
    _lighting->getTechnique(technique)
        ->getParameter("u_spotLightInnerAngleCos[0]")
        ->setValue(_spotLightNode->getLight()->getInnerAngleCos());
    _lighting->getTechnique(technique)
        ->getParameter("u_spotLightOuterAngleCos[0]")
        ->setValue(_spotLightNode->getLight()->getOuterAngleCos());
    _lighting->getTechnique(technique)
        ->getParameter("u_spotLightRangeInverse[0]")
        ->setValue(_spotLightNode->getLight()->getRangeInverse());
    _lighting->getTechnique(technique)
        ->getParameter("u_spotLightDirection[0]")
        ->bindValue(_spotLightNode, &Node::getForwardVectorView);
    _lighting->getTechnique(technique)
        ->getParameter("u_spotLightPosition[0]")
        ->bindValue(_spotLightNode, &Node::getTranslationView);
}
...
void LightSample::initializePointTechnique(const std::string& technique)
{
    _lighting->getTechnique(technique)
        ...
        ->setValue(Vector3(0.0f, 0.0f, 0.0f));
    _lighting->getTechnique(technique)
        ->getParameter("u_pointLightColor[0]")
        ->setValue(Vector3(_redSlider->getValue(), _greenSlider->getValue(), _blueSlider->getValue()));
    _lighting->getTechnique(technique)
        ->getParameter("u_pointLightPosition[0]")
        ->bindValue(_pointLightNode, &Node::getTranslationView);
    _lighting->getTechnique(technique)
        ->getParameter("u_pointLightRangeInverse[0]")
        ->setValue(_pointLightNode->getLight()->getRangeInverse());
}
...
void LightSample::setUnlitMaterialTexture(Model* model, const std::string& texturePath, bool mipmap)
{
    ...
    Texture::Sampler* sampler =
        material->getParameter("u_diffuseTexture")->setValue(texturePath, mipmap);
    if (mipmap)
    {
...
void LightSample::setColorValue(const Vector3& value)
{
    ...
    _lighting->getTechnique(("directionalSpecular"))
        ->getParameter("u_directionalLightColor[0]")
        ->setValue(value);
    _lighting->getTechnique(("directionalBumped"))
        ->getParameter("u_directionalLightColor[0]")
        ->setValue(value);
    _lighting->getTechnique(("directionalSpecularBumped"))
        ->getParameter("u_directionalLightColor[0]")
        ->setValue(value);
...
void LightSample::setSpecularValue(float value)
{
    ...
    _lighting->getTechnique("directionalSpecularBumped")
        ->getParameter("u_specularExponent")
        ->setValue(value);
