...
std::string ParticlesSample::toString(bool b) { return b ? "true" : "false"; }
...
{
    std::string filename;
    filename = FileSystem::displayFileDialog(FileSystem::SAVE,
                                             "Save Particle File",
                                             "Particle Files",
                                             "particle",
                                             "res");
    ...
    switch (evt)
    {
        case Listener::VALUE_CHANGED:
            if (control == _startRed)
            {
                ...
                emitter->setColor(startColor,
                                  emitter->getColorStartVariance(),
                                  emitter->getColorEnd(),
            ...
            else if (control == _endRed)
            {
                ...
                emitter->setColor(emitter->getColorStart(),
                                  emitter->getColorStartVariance(),
                                  endColor,
                                  emitter->getColorEndVariance());
            }
            ...
            {
                emitter->setSize(_startMin->getValue(),
                                 emitter->getSizeStartMax(),
                                 emitter->getSizeEndMin(),
                                 emitter->getSizeEndMax());
            }
            else if (control == _startMax)
            {
                emitter->setSize(emitter->getSizeStartMin(),
                                 _startMax->getValue(),
                                 emitter->getSizeEndMin(),
                                 emitter->getSizeEndMax());
            }
            ...
            {
                emitter->setRotationPerParticle(_spinSpeedMin->getValue(),
                                                emitter->getRotationPerParticleSpeedMax());
            }
            else if (control == _spinSpeedMax)
            {
                emitter->setRotationPerParticle(emitter->getRotationPerParticleSpeedMin(),
                                                _spinSpeedMax->getValue());
            }
            else if (control == _axisX)
            {
                Vector3 axis = emitter->getRotationAxis();
                axis.x = _axisX->getValue();
                emitter->setRotation(emitter->getRotationSpeedMin(),
                                     emitter->getRotationSpeedMax(),
                                     axis,
                                     emitter->getRotationAxisVariance());
            }
            ...
            {
                emitter->setRotation(_rotationSpeedMin->getValue(),
                                     emitter->getRotationSpeedMax(),
                                     emitter->getRotationAxis(),
                                     emitter->getRotationAxisVariance());
            }
        ...
        case Listener::RELEASE:
            ...
            {
                Game::getInstance()->pause();
                std::string filename = FileSystem::displayFileDialog(FileSystem::OPEN,
...
void ParticlesSample::update(float elapsedTime)
{
    ...
    ParticleEmitter* emitter = dynamic_cast<ParticleEmitter*>(_particleEmitterNode->getDrawable());
    if (emitter) emitter->update(elapsedTime);
}
            ...
            switch (key)
            {
                ...
                case Keyboard::KEY_B:
                {
                    ParticleEmitter* emitter =
                        dynamic_cast<ParticleEmitter*>(_particleEmitterNode->getDrawable());
                    emitter->setBlendMode(ParticleEmitter::BLEND_NONE);
                    break;
                }
                case Keyboard::KEY_W:
...
{
    setViewport(tractor::Rectangle(width, height));
