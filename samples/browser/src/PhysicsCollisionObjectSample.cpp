...
PhysicsCollisionObjectSample::PhysicsCollisionObjectSample()
    : _font(nullptr), _scene(nullptr), _lightNode(nullptr), _form(nullptr), _objectType(SPHERE),
      _throw(true), _drawDebug(0), _wireFrame(false)
{
    const std::string paths[] = { "res/common/physics.physics#ball",
                                  "res/common/physics.physics#box",
                                  "res/common/physics.physics#capsule",
                                  "res/common/physics.physics#duck" };
    _collisionObjectPaths.assign(paths, paths + 4);
    const std::string nodeIds[] = { "sphere", "box", "capsule", "duck" };
    _nodeIds.assign(nodeIds, nodeIds + 4);
    const std::string nodeNames[] = { "Sphere", "Box", "Capsule", "Duck" };
    _nodeNames.assign(nodeNames, nodeNames + 4);
    Vector4 colors[] = { Vector4(1, 0, 0, 1),
                         Vector4(0.1f, 0.6f, 0.1f, 1),
                         Vector4(0, 0, 1, 1),
                         Vector4(1, 1, 0, 1) };
...
void PhysicsCollisionObjectSample::initialize()
{
    ...
    static_cast<Button*>(_form->getControl("wireframeButton"))
        ->addListener(this, Control::Listener::CLICK);
    static_cast<Button*>(_form->getControl("drawDebugButton"))
        ->addListener(this, Control::Listener::CLICK);
...
void PhysicsCollisionObjectSample::touchEvent(Touch::TouchEvent evt,
                                              int x,
                                              int y,
...
bool PhysicsCollisionObjectSample::drawScene(Node* node)
{
    ...
    if (drawable) drawable->draw(_wireFrame);
    return true;
}
...
void PhysicsCollisionObjectSample::fireProjectile(const Ray& ray)
{
    ...
    // It is important to set the transform before attaching the collision object because this rigid
    // body is not kinematic. Once the non-kinematic rigid body is attached, you should only move
    // the object using forces.
    PhysicsCollisionObject* collisionObject =
        clone->setCollisionObject(_collisionObjectPaths[_objectType]);
...
void PhysicsCollisionObjectSample::incrementDebugDraw()
{
    ...
    static_cast<Button*>(_form->getControl("drawDebugButton"))
        ->setText(_drawDebug == 0 ? "Normal" : "Debug");
}
...
void PhysicsCollisionObjectSample::toggleWireframe()
{
    ...
    static_cast<Button*>(_form->getControl("wireframeButton"))
        ->setText(_wireFrame ? "Wireframe" : "Solid");
}
...
void PhysicsCollisionObjectSample::controlEvent(Control* control, EventType evt)
{
    ...
    if (button->getId() == "wireframeButton")
    {
        toggleWireframe();
    }
    else if (button->getId() == "drawDebugButton")
    {
        incrementDebugDraw();
    }
    else if (button->getId() == "throwButton")
    {
        _throw = !_throw;
        button->setText(_throw ? "Throw" : "Drop");
    }
    else if (button->getId() == "shapeButton")
    {
