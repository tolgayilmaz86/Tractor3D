...
void PostProcessSample::initialize()
{
    ...
    compositor = Compositor::create(_frameBuffer,
                                    nullptr,
    ...
    compositor = Compositor::create(_frameBuffer,
                                    ...
                                    "Gaussian Blur");
    _compositors.push_back(compositor);
    compositor->getMaterial()
        ->getParameter("u_length")
        ->setValue(1.0f / ((float)FRAMEBUFFER_WIDTH / 2.0f));
...
void PostProcessSample::update(float elapsedTime)
{
    ...
    if (_compositorIndex == _compositors.size() - 1)
    {
        ...
        if (elapsedParam) elapsedParam->setValue(elapsedTime);
        MaterialParameter* randomParam = compositor->getMaterial()->getParameter("u_random");
        if (randomParam) randomParam->setValue(MATH_RANDOM_0_1());
    }
}
...
void PostProcessSample::render(float elapsedTime)
{
    ...
    // Draw the pass through compositor at index 0 at quarter of the size and bottom right. dont
    // clear the dest just draw last on top
...
void PostProcessSample::drawTechniqueId(const std::string& techniqueId)
{
    char buffer[128];
    sprintf(buffer, "%s", techniqueId.c_str());
    _font->start();
    _font->drawText(buffer,
                    Rectangle(0, 10, getWidth(), getHeight()),
                    Vector4::one(),
                    18,
                    Font::ALIGN_TOP_HCENTER);
    _font->finish();
}
...
void PostProcessSample::touchEvent(Touch::TouchEvent evt, int x, int y, unsigned int contactIndex)
{
    switch (evt)
    {
        ...
        {
            _compositorIndex++;
            if (_compositorIndex == _compositors.size()) _compositorIndex = 0;
        }
        break;
