...
bool Sample::isVsync() { return Game::isVsync(); }

void Sample::setVsync(bool enable) { Game::setVsync(enable); }

long Sample::getAbsoluteTime() { return Game::getAbsoluteTime(); }

long Sample::getGameTime() { return Game::getGameTime(); }

Game::State Sample::getState() const { return Game::getInstance()->getState(); }

int Sample::run() { return Game::getInstance()->run(); }

void Sample::pause() { Game::getInstance()->pause(); }

void Sample::resume() { Game::getInstance()->resume(); }

void Sample::exit() { Game::getInstance()->exit(); }

void Sample::frame() { Game::getInstance()->frame(); }

unsigned int Sample::getFrameRate() const { return Game::getInstance()->getFrameRate(); }

const Rectangle& Sample::getViewport() const { return Game::getInstance()->getViewport(); }

void Sample::setViewport(const Rectangle& viewport) { Game::getInstance()->setViewport(viewport); }

unsigned int Sample::getWidth() const { return Game::getInstance()->getWidth(); }

unsigned int Sample::getHeight() const { return Game::getInstance()->getHeight(); }

float Sample::getAspectRatio() const { return Game::getInstance()->getAspectRatio(); }
...
void Sample::clear(Game::ClearFlags flags,
                   float red,
                   float green,
                   float blue,
                   float alpha,
...
void Sample::displayKeyboard(bool display) { Game::getInstance()->displayKeyboard(display); }
...
bool Sample::mouseEvent(Mouse::MouseEvent evt, int x, int y, int wheelDelta) { return false; }

bool Sample::hasMouse() { return Game::getInstance()->hasMouse(); }

bool Sample::isMouseCaptured() { return Game::getInstance()->isMouseCaptured(); }

void Sample::setMouseCaptured(bool captured) { Game::getInstance()->setMouseCaptured(captured); }

void Sample::setMultiTouch(bool enabled) { Game::getInstance()->setMultiTouch(enabled); }

bool Sample::isMultiTouch() const { return Game::getInstance()->isMultiTouch(); }

bool Sample::hasAccelerometer() const { return Game::getInstance()->hasAccelerometer(); }
...
void Sample::getSensorValues(float* accelX,
                             float* accelY,
                             float* accelZ,
                             float* gyroX,
                             float* gyroY,
...
{
    Game::getInstance()->getScriptController()->executeFunction<void>("camera_setActive",
                                                                      "b",
                                                                      nullptr,
                                                                      enable);
}
...
{
    Game::getInstance()->getScriptController()->executeFunction<void>("camera_setSpeed",
                                                                      "ff",
                                                                      nullptr,
                                                                      normal,
                                                                      fast);
...
unsigned int Sample::getGamepadCount() const { return Game::getInstance()->getGamepadCount(); }
