...
void SamplesGame::touchEvent(Touch::TouchEvent evt, int x, int y, unsigned int contactIndex)
{
    if (_activeSample)
    {
        ...
        {
            getScriptController()->executeFunction<void>("camera_touchEvent",
                                                         "[Touch::TouchEvent]iiui",
                                                         nullptr,
                                                         evt,
                                                         x,
                                                         y,
...
void SamplesGame::keyEvent(Keyboard::KeyEvent evt, int key)
{
    if (_activeSample)
    {
        if (key == Keyboard::KEY_MENU
            || (evt == Keyboard::KEY_PRESS && (key == Keyboard::KEY_ESCAPE)))
        {
        ...
        {
            getScriptController()->executeFunction<void>("camera_keyEvent",
                                                         "[Keyboard::KeyEvent][Keyboard::Key]",
                                                         nullptr,
                                                         evt,
                                                         key);
...
void SamplesGame::menuEvent() { exitActiveSample(); }

void SamplesGame::gestureSwipeEvent(int x, int y, int direction)
{
    if (_activeSample) _activeSample->gestureSwipeEvent(x, y, direction);
}

void SamplesGame::gesturePinchEvent(int x, int y, float scale)
{
    if (_activeSample) _activeSample->gesturePinchEvent(x, y, scale);
}

void SamplesGame::gestureTapEvent(int x, int y)
{
    if (_activeSample) _activeSample->gestureTapEvent(x, y);
}

void SamplesGame::gestureLongTapEvent(int x, int y, float duration)
{
    if (_activeSample) _activeSample->gestureLongTapEvent(x, y, duration);
}

void SamplesGame::gestureDragEvent(int x, int y)
{
    if (_activeSample) _activeSample->gestureDragEvent(x, y);
}

void SamplesGame::gestureDropEvent(int x, int y)
{
    if (_activeSample) _activeSample->gestureDropEvent(x, y);
}
...
{
    if (_activeSample) _activeSample->gamepadEvent(evt, gamepad);
}
...
void SamplesGame::addSample(const std::string& category,
                            const std::string& title,
                            void* func,
                            unsigned int order)
{
    if (_samples == nullptr) _samples = new std::vector<SampleRecordList>();
...
SamplesGame* SamplesGame::getInstance() { return &game; }
