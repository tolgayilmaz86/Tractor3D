...
{
    TerrainHitFilter(Terrain* terrain) { terrainObject = terrain->getNode()->getCollisionObject(); }
...
TerrainSample::TerrainSample()
    : _font(nullptr), _scene(nullptr), _terrain(nullptr), _sky(nullptr), _form(nullptr),
      _formVisible(true), _wireframe(false), _debugPhysics(false), _snapToGround(true),
      _vsync(true), _mode(MODE_LOOK), _sphere(nullptr), _box(nullptr), _directionalLight(nullptr)
{
}
...
bool TerrainSample::drawScene(Node* node)
{
    ...
    {
        if (!node->getBoundingSphere().intersects(camera->getFrustum())) return true;
    }
    if (drawable)
    {
        ...
        else if (_mode != MODE_LOOK)
        {
                ...
                && hitResult.object == _terrain->getNode()->getCollisionObject())
            {
                ...
                if (clone)
                {
                    ...
                    clone->setTranslation(hitResult.point.x,
                                          hitResult.point.y + 50,
                                          hitResult.point.z);
...
{
    if (autoBinding == "LIGHT_DIRECTION_0")
    {
        parameter->bindValue(this, &TerrainSample::getLightDirection0);
        return true;
    }
    else if (autoBinding == "LIGHT_COLOR_0")
    {
