...
WaterSample::WaterSample()
    : _font(nullptr), _scene(nullptr), _cameraNode(nullptr), _reflectCameraNode(nullptr),
      _inputMask(0u), _prevX(0), _prevY(0), _waterHeight(0.f), _refractBuffer(nullptr),
      _refractBatch(nullptr), _reflectBuffer(nullptr), _reflectBatch(nullptr), _showBuffers(false),
      _gamepad(nullptr)
{
}
...
void WaterSample::update(float elapsedTime)
{
    ...
    if (_gamepad && _gamepad->getJoystickCount()) _gamepad->getJoystickValues(0, &axis);
    ...
    if (_inputMask & MOVE_BACKWARD || axis.y < -minVal)
        force -= _cameraNode->getFirstChild()->getForwardVectorWorld();
    if (_inputMask & MOVE_LEFT || axis.x < -minVal) force += _cameraNode->getRightVectorWorld();
    if (_inputMask & MOVE_RIGHT || axis.y > minVal) force -= _cameraNode->getRightVectorWorld();
    if (force.lengthSquared() > 1.f) force.normalize();

    _cameraAcceleration += force / MASS;
    _cameraAcceleration *= FRICTION;
    if (_cameraAcceleration.lengthSquared() < 0.01f) _cameraAcceleration = Vector3::zero();
...
void WaterSample::render(float elapsedTime)
{
    ...
    {
        _refractBatch->start();
        _refractBatch->draw(Vector3(0.f, 4.f, 0.f),
                            Rectangle(BUFFER_SIZE, BUFFER_SIZE),
                            Vector2(426.f, 240.f));
        _refractBatch->finish();
        _reflectBatch->start();
        _reflectBatch->draw(Vector3(430.f, 4.f, 0.f),
                            Rectangle(BUFFER_SIZE, BUFFER_SIZE),
                            Vector2(426.f, 240.f));
