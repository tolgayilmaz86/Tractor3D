...
void CharacterGame::initialize()
{
    ...
    ceiling->setCollisionObject(PhysicsCollisionObject::RIGID_BODY,
                                PhysicsCollisionShape::box(Vector3(49.5f, 1.0f, 49.5f)),
                                &rbParams);
...
void CharacterGame::initializeMaterial(Scene* scene, Node* node, Material* material)
{
    ...
    if (node->hasTag("dynamic"))
    {
        ...
        if (lightNode)
        {
            material->getParameter("u_directionalLightColor[0]")
                ->bindValue(lightNode->getLight(), &Light::getColor);
...
void CharacterGame::initializeCharacter()
{
    ...
    _materialParameterAlpha = dynamic_cast<Model*>(_characterMeshNode->getDrawable())
                                  ->getMaterial()
                                  ->getTechniqueByIndex(0)
                                  ->getPassByIndex(0)
                                  ->getParameter("u_modulateAlpha");
...
void CharacterGame::play(const char* id, bool repeat, float speed)
{
    ...
    if (clip == _currentClip && clip->isPlaying()) return;
...
void CharacterGame::update(float elapsedTime)
{
    ...
    if (_currentDirection.isZero())
    {
        ...
        if ((_keyFlags & RUNNING) == 0) _currentDirection *= 0.5f;
    }
    ...
    if (getPhysicsController()->rayTest(Ray(Vector3(v.x, v.y + 1.0f, v.z), Vector3(0, -1, 0)),
                                        100.0f,
                                        &hitResult,
                                        nullptr))
    {
        _characterShadowNode->setTranslation(
            Vector3(hitResult.point.x, hitResult.point.y + 0.1f, hitResult.point.z));
    }

    if (_hasBall)
    {
        // This is the first time entering this block of code if the basketball is still enabled.
        // Disable physics on basketball, and create a bigger collision object around the boy to
        ...
        Vector3 translation(_characterNode->getTranslationWorld()
                            + characterForwardVector.normalize() * -2.2f);
...
void CharacterGame::adjustCamera(float elapsedTime)
{
    ...
    do
    {
        ...
        if (!physics->sweepTest(cameraNode->getCollisionObject(), focalPoint, &result)
            || result.object == _character)
            break;
        ...
        cameraOffset += d;
        while (physics->sweepTest(cameraNode->getCollisionObject(), focalPoint, &result)
               && result.object == occlusion)
        {
            ...
            if (cameraNode->getTranslationWorld().distanceSquared(focalPoint) <= 2.0f) return;
    ...
    // If the character is closer than 10 world units to the camera, apply transparency to the
    // character so he does not obstruct the view.
    if (occlusion)
    {
        float d = _scene->getActiveCamera()->getNode()->getTranslationWorld().distance(
            _characterNode->getTranslationWorld());
        float alpha = d < 10 ? (d * 0.1f) : 1.0f;
        if (alpha < 1.0f)
        {
            _characterMeshNode->setTag("transparent", "true");
        }
        else
        {
            _characterMeshNode->removeTag("transparent");
        }
                                   ...
                                   const Vector3& contactPointB)
{
    ...
    if (type == PhysicsCollisionObject::CollisionListener::COLLIDING
        && collisionPair.objectB == _character)
        _hasBall = true;
}
...
void CharacterGame::grabBall()
{
    ...
    boy->setCollisionObject(PhysicsCollisionObject::CHARACTER,
                            PhysicsCollisionShape::capsule(2.9f, 6.0f, Vector3(0.0f, 3.0f, 0.0f), true),
                            &rbParams);
...
void CharacterGame::releaseBall()
{
    ...
    Node* boy = _character->getNode();
    boy->setCollisionObject(PhysicsCollisionObject::CHARACTER,
                            PhysicsCollisionShape::capsule(1.2f, 6.0f, Vector3(0.0f, 3.0f, 0.0f), true),
                            &rbParams);
    _character = static_cast<PhysicsCharacter*>(boy->getCollisionObject());
    _character->setVelocity(velocity);
    _character->setMaxSlopeAngle(0.0f);
