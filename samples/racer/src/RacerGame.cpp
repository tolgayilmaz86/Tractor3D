...
bool RacerGame::initializeScene(Node* node)
{
    ...
    if (model)
    {
        ...
        if (material
            && material->getTechnique()->getPassByIndex(0)->getEffect()->getUniform(
    ...
    if (_scene->getActiveCamera() && (cameraNode = _scene->getActiveCamera()->getNode()))
    {
        ...
        if (_carVehicle)
        {
            ...
            if (!__flythruCamera)
            {
                ...
                if (!driving
                    && ((_keyFlags & ACCELERATOR) || (_keyFlags & ACCELERATOR_MOUSE)
                        || _gamepad->isButtonDown(Gamepad::BUTTON_A)))
                {
                    driving = 1;
                ...
                bool isReverseCommanded =
                    (_keyFlags & REVERSE) || (!isVirt && _gamepad->isButtonDown(Gamepad::BUTTON_X))
                ...
                if ((_keyFlags & BRAKE) || (_keyFlags & BRAKE_MOUSE)
                    || _gamepad->isButtonDown(Gamepad::BUTTON_B)
                    || (_gamepad->getTriggerCount() > 0 && _gamepad->getTriggerValue(0) > 0.5f))
                {
                    braking = 1;
                    if (_brakingSound && (_brakingSound->getState() != AudioSource::PLAYING)
                        && (v > 30.0f))
                        _brakingSound->play();
                }
                else
                ...
                Vector3 commandedPosition(carPosition + Vector3::unitY() * 4.0f
                                          - carNode->getBackVector() * 10.0f);
                cameraNode->translateSmooth(commandedPosition, dt, 0.2f);
                Matrix m =
                    Matrix::createLookAt(cameraNode->getTranslation(), carPosition, Vector3::unitY());
                m.transpose();
                Quaternion q;
                m.getRotation(&q);
            ...
            else if ((_keyFlags & UPRIGHT) || (!isVirt && _gamepad->isButtonDown(Gamepad::BUTTON_Y))
...
void RacerGame::render(float elapsedTime)
{
    ...
    if (__drawDebug)
    {
        Game::getInstance()->getPhysicsController()->drawDebug(
            _scene->getActiveCamera()->getViewProjectionMatrix());
    }

    // Draw the gamepad
    if (_gamepad && _gamepad->isVirtual()) _gamepad->draw();
...
bool RacerGame::buildRenderQueues(Node* node)
{
    ...
    if (model)
    {
        ...
        if (__viewFrustumCulling
            && !node->getBoundingSphere().intersects(_scene->getActiveCamera()->getFrustum()))
            return true;
...
void RacerGame::gamepadEvent(Gamepad::GamepadEvent evt, Gamepad* gamepad)
{
    ...
    switch (evt)
    {
        case Gamepad::CONNECTED_EVENT:
            ...
            {
                if (!_physicalGamepad) _physicalGamepad = gamepad;
            }
...
{
    if (control->getId() == "newGameButton")
    {
        resetToStart();
        // Close the menu and resume the game.
        menuEvent();
    }
    else if (std::string(control->getId()) == "quitGameButton")
    {
        exit();
    }
    else if (std::string(control->getId()) == "useGamepad")
    {
        __useAccelerometer = false;
    }
    else if (std::string(control->getId()) == "useTilt")
    {
        __useAccelerometer = true;
    }
    else if (std::string(control->getId()) == "menuButton")
    {
        menuEvent();
    }
}
