...
// Gravity constant (earth's gravity is ~9.8m/s/s)//;x'
#define GRAVITY 9.8f
...
void SpaceshipGame::initialize()
{
    ...
    if (_backgroundMusic) _backgroundMusic->setLooped(true);
...
void SpaceshipGame::initializeSpaceship()
{
    ...
    material = dynamic_cast<Model*>(_shipNode->getDrawable())
                   ->setMaterial("res/shaders/colored.vert",
                                 "res/shaders/colored.frag",
                                 "SPECULAR;DIRECTIONAL_LIGHT_COUNT 1",
                                 0);
    ...
    material = dynamic_cast<Model*>(_propulsionNode->getDrawable())
                   ->setMaterial("res/shaders/colored.vert",
                                 "res/shaders/colored.frag",
                                 "SPECULAR;DIRECTIONAL_LIGHT_COUNT 1");
    material->getParameter("u_diffuseColor")->setValue(Vector4(0.8f, 0.8f, 0.8f, 1.0f));
    ...
    material =
        dynamic_cast<Model*>(_glowNode->getDrawable())
            ->setMaterial("res/shaders/textured.vert", "res/shaders/textured.frag", "MODULATE_COLOR");
...
void SpaceshipGame::initializeEnvironment()
{
    ...
    for (size_t i = 0, count = nodes.size(); i < count; ++i)
    {
        ...
        material = dynamic_cast<Model*>(pGround->getDrawable())
                       ->setMaterial("res/shaders/colored.vert",
                                     "res/shaders/colored.frag",
                                     "SPECULAR;DIRECTIONAL_LIGHT_COUNT 1");
        material->getParameter("u_diffuseColor")->setValue(Vector4(0.280584f, 0.5584863f, 0.6928f, 1.0f));
    ...
    {
        Node* pRoof = nodes[i];
        material = dynamic_cast<Model*>(pRoof->getDrawable())
                       ->setMaterial("res/shaders/colored.vert",
                                     "res/shaders/colored.frag",
                                     "SPECULAR;DIRECTIONAL_LIGHT_COUNT 1");
        material->getParameter("u_diffuseColor")->setValue(Vector4(0.280584f, 0.5584863f, 0.6928f, 1.0f));
    ...
    material = dynamic_cast<Model*>(pBackground->getDrawable())
                   ->setMaterial("res/shaders/textured.vert",
    ...
    if (lighting)
    {
        ...
        if (lightNode)
        {
            material->getParameter("u_directionalLightColor[0]")
                ->setValue(lightNode->getLight()->getColor());
...
void SpaceshipGame::update(float elapsedTime)
{
    ...
    if (!_finished)
    {
        ...
        if (_backgroundMusic->getState() != AudioSource::PLAYING) _backgroundMusic->play();
    }
    else
    {
        // Stop the background music track
        if (_backgroundMusic->getState() == AudioSource::PLAYING
            || _backgroundMusic->getState() == AudioSource::PAUSED)
        {
    ...
    // While we are pushing/touching the screen, apply a push force vector based on the distance
    // from the touch point to the center of the space ship.
    if (_pushing)
    {
    ...
    if (_throttle > MATH_EPSILON)
    {
        ...
        // Play sound effect
        if (_spaceshipSound->getState() != AudioSource::PLAYING) _spaceshipSound->play();
...
void SpaceshipGame::handleCollisions(float t)
{
    ...
    BoundingBox propulsionBounds =
        dynamic_cast<Model*>(_propulsionNode->getDrawable())->getMesh()->getBoundingBox();
...
void SpaceshipGame::drawText()
{
    ...
    if (_finished)
    {
        _font->drawText("Click to Play Again",
                        getWidth() / 2 - 175,
                        getHeight() / 2 - 40,
                        Vector4::one(),
                        _font->getSize());
